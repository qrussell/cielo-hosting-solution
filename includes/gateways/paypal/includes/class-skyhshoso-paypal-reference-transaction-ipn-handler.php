<?php
/**
 * SkyHS Hosting Solution Handles responses from PayPal IPN for Reference Transactions
 *
...
class SkyHSHOSO_PayPal_Reference_Transaction_IPN_Handler extends SkyHSHOSO_PayPal_Standard_IPN_Handler {
	...
	protected function cancel_subscriptions( $billing_agreement_id ) {
		$note = esc_html__( 'Billing agreement cancelled at PayPal.', 'skyhs-hosting-solution' );

		foreach ( SkyHSHOSO_PayPal::get_subscriptions_by_paypal_id( $billing_agreement_id, 'objects' ) as $subscription ) {
			...
			if ( $is_paypal_subscription && ! in_array( $subscription->get_status(), array( 'cancelled', 'expired' ) ) ) {
				try {
