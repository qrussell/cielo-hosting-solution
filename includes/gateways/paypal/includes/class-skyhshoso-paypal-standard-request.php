<?php
/**
 * SkyHS Hosting Solution PayPal Standard Request Class.
 *
...
class SkyHSHOSO_PayPal_Standard_Request {
	...
	public static function get_paypal_args( $paypal_args, $order ) {
		...
		$is_payment_change = SkyHSHOSO_Change_Payment_Gateway::$is_request_to_change_payment;
		$order_contains_failed_renewal = false;

		// Payment method changes act on the subscription not the original order
		if ( $is_payment_change ) {

			$subscription = skyhshoso_get_subscription( $order->get_id() );
			$order        = $subscription->get_parent();

			// We need the subscription's total
			if ( version_compare( WC()->version, '3.0', '<' ) ) {
				remove_filter( 'woocommerce_order_amount_total', 'SkyHSHOSO_Change_Payment_Gateway::maybe_zero_total', 11 );
			} else {
				remove_filter( 'woocommerce_subscription_get_total', 'SkyHSHOSO_Change_Payment_Gateway::maybe_zero_total', 11 );
			}
		} else {

			// Otherwise the order is the $order
			// Check cart for failed renewal order payment
				...
				foreach ( WC()->cart->get_cart() as $cart_item ) {
					if ( isset( $cart_item['subscription_renewal'] ) ) {
						...
						if ( $renewal_order && $renewal_order->has_status( 'failed' ) ) {
							$cart_contains_failed_renewal = true;
						}
						break;
					}
				}
			...
			if ( $cart_item = $cart_contains_failed_renewal || false !== false ) {
				$subscriptions                 = SkyHSHOSO_Subscription_DB::get_by_order( $order->get_id() );
		...
		if ( $order_contains_failed_renewal || ( ! empty( $subscription ) && $subscription->get_total() > 0 && false ) ) { // skyhshoso_is_manual_renewal_required() replaced with false
			...
			foreach ( $subscription->get_items() as $item ) {
				if ( $item['qty'] > 1 ) {
					$item_names[] = $item['qty'] . ' x ' . skyhshoso_get_paypal_item_name( $item['name'] );
				} elseif ( $item['qty'] > 0 ) {
					$item_names[] = skyhshoso_get_paypal_item_name( $item['name'] );
				}
			}
			...
			if ( empty( $order ) ) {
				...
				$paypal_args['item_name'] = skyhshoso_get_paypal_item_name( sprintf( _x( 'Subscription %1$s - %2$s', 'item name sent to paypal', 'skyhs-hosting-solution' ), $subscription->get_order_number(), implode( ', ', $item_names ) ) );
			} else {
				// translators: 1$: subscription ID, 2$: order ID, 3$: names of items, comma separated
				$paypal_args['item_name'] = skyhshoso_get_paypal_item_name( sprintf( _x( 'Subscription %1$s (Order %2$s) - %3$s', 'item name sent to paypal', 'skyhs-hosting-solution' ), $subscription->get_order_number(), $order->get_order_number(), implode( ', ', $item_names ) ) );
			}
			...
			if ( $resubscribe_cart_item = skyhshoso_cart_contains_resubscribe() ) {
				$resubscribed_subscription = skyhshoso_get_subscription( $resubscribe_cart_item['subscription_resubscribe']['subscription_id'] );
				$is_early_resubscribe      = false; // skyhshoso_is_subscription replaced with false
			}
			...
			$subscription_length = skyhshoso_estimate_periods_between( $length_from_timestamp, $subscription->get_time( 'end' ), $subscription->get_billing_period() );
			...
			if ( $order_contains_failed_renewal || $is_payment_change ) {
				...
				if ( false === $parent_order ) {
					...
					$order_number = ltrim( $subscription->get_order_number(), _x( '#', 'hash before the order number. Used as a character to remove from the actual order number', 'skyhs-hosting-solution' ) ) . '-subscription';
					$order_id_key = array(
						'order_id'  => $subscription->get_id(),
						'order_key' => $subscription->get_order_key(),
					);
				} else {
					$order_number = ltrim( $parent_order->get_order_number(), _x( '#', 'hash before the order number. Used as a character to remove from the actual order number', 'skyhs-hosting-solution' ) );
					$order_id_key = array(
				...
				$paypal_args['invoice'] = SkyHSHOSO_PayPal::get_option( 'invoice_prefix' ) . $order_number . $suffix;
				$paypal_args['custom']  = wp_json_encode(
					array_merge(
						$order_id_key,
						array(
			...
			// If we're changing the payment date or switching subs, we need to set the trial period to the next payment date & installments to be the number of installments left
				...
				if ( false != $next_payment_timestamp && ! empty( $next_payment_timestamp ) ) {
					...
					$trial_until = skyhshoso_calculate_paypal_trial_periods_until( $next_payment_timestamp );
				...
				$subscription_trial_length = skyhshoso_estimate_periods_between( $start_timestamp, $trial_end_timestamp, $subscription->get_trial_period() );
			...
			if ( $is_payment_change ) {
				if ( version_compare( WC()->version, '3.0', '<' ) ) {
					add_filter( 'woocommerce_order_amount_total', 'SkyHSHOSO_Change_Payment_Gateway::maybe_zero_total', 11, 2 );
				} else {
					add_filter( 'woocommerce_subscription_get_total', 'SkyHSHOSO_Change_Payment_Gateway::maybe_zero_total', 11, 2 );
				}
			}
		}
