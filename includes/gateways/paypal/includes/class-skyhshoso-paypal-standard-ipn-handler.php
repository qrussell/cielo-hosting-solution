<?php
/**
 * SkyHS Hosting Solution PayPal Standard IPN Handler
 *
...
class SkyHSHOSO_PayPal_Standard_IPN_Handler extends WC_Gateway_Paypal_IPN_Handler {
	...
	protected function process_ipn_request( $transaction_details ) {
		...
		$subscription            = skyhshoso_get_subscription( $subscription_id_and_key['order_id'] );
		...
		remove_filter( 'woocommerce_subscription_payment_gateway_supports', 'SkyHSHOSO_PayPal_Supports::add_feature_support_for_subscription', 10 );
		...
		if ( ! is_callable( array( $subscription, 'get_id' ) ) ) {
			$subscription = skyhshoso_get_subscription( wc_get_order_id_by_order_key( $subscription_key ) );
		}
		...
		if ( isset( $transaction_details['txn_id'] ) ) {
			...
			$ipn_lock_transient_name = 'skyhshoso_pp_' . md5( $ipn_transaction_id ); // transient names need to be less than 45 characters and the $ipn_id will be long, e.g. 34292625HU746553V_subscr_payment_completed_5ab4c38e1f39d, so md5
		...
		if ( in_array( $transaction_details['txn_type'], array( 'subscr_signup', 'subscr_payment' ) ) && false !== strpos( $transaction_details['invoice'], '-wcsfrp-' ) ) {
			...
			if ( $transaction_order->get_id() != get_post_meta( $subscription->get_id(), '_paypal_failed_sign_up_recorded', true ) ) {
		...
		if ( $is_renewal_sign_up_after_failure || $is_payment_change ) {
			...
			$existing_profile_id = skyhshoso_get_paypal_id( $subscription );
		...
		if ( isset( $transaction_details['subscr_id'] ) && ! in_array( $transaction_details['txn_type'], array( 'subscr_cancel', 'subscr_eot' ) ) ) {
			skyhshoso_set_paypal_id( $subscription, $transaction_details['subscr_id'] );

			if ( skyhshoso_is_paypal_profile_a( $transaction_details['subscr_id'], 'out_of_date_id' ) && 'disabled' != get_option( 'skyhshoso_paypal_invalid_profile_id' ) ) {
				update_option( 'skyhshoso_paypal_invalid_profile_id', 'yes' );
			}
		}
		...
		switch ( $transaction_details['txn_type'] ) {
			case 'subscr_signup':
				...
				if ( $is_payment_change ) {
					...
					SkyHSHOSO_Change_Payment_Gateway::update_payment_method( $subscription, 'paypal' );
					...
					$this->add_order_note( _x( 'IPN subscription payment method changed to PayPal.', 'when it is a payment change, and there is a subscr_signup message, this will be a confirmation message that PayPal accepted it being the new payment method', 'skyhs-hosting-solution' ), $subscription, $transaction_details );

				} else {

					$this->add_order_note( __( 'IPN subscription sign up completed.', 'skyhs-hosting-solution' ), $subscription, $transaction_details );
			...
			case 'subscr_payment':
				...
				if ( ! $is_first_payment && ! $is_renewal_sign_up_after_failure ) {
					...
					if ( $subscription->has_status( 'active' ) ) {
						remove_action( 'skyhshoso_subscription_on-hold_paypal', 'SkyHSHOSO_PayPal_Status_Manager::suspend_subscription' );
						$subscription->update_status( 'on-hold' );
						add_action( 'skyhshoso_subscription_on-hold_paypal', 'SkyHSHOSO_PayPal_Status_Manager::suspend_subscription' );
					}
					...
					if ( is_null( $transaction_order ) ) {
						$transaction_order = skyhshoso_create_renewal_order( $subscription );
					}
				...
				if ( 'completed' == strtolower( $transaction_details['payment_status'] ) ) {
					...
					$this->add_order_note( __( 'IPN subscription payment completed.', 'skyhs-hosting-solution' ), $subscription, $transaction_details );
					...
					if ( $is_first_payment ) {
						...
						if ( ! $parent_order ) {
							$parent_order = skyhshoso_create_renewal_order( $subscription );
						}
					...
					} elseif ( $subscription->get_payment_count() == 1 && '' !== SkyHSHOSO_PayPal::get_option( 'identity_token' ) && 'true' != get_post_meta( $subscription->get_id(), '_paypal_first_ipn_ignored_for_pdt', true ) && false === $is_renewal_sign_up_after_failure ) {
					...
					// Process the payment if the subscription is active
						...
						if ( true === $is_renewal_sign_up_after_failure && is_object( $transaction_order ) ) {
							...
							update_post_meta( $subscription->get_id(), '_paypal_failed_sign_up_recorded', $transaction_order->get_id() );
							...
							if ( 'paypal' == get_post_meta( $subscription->get_id(), '_old_payment_method', true ) ) {
								...
								$this->add_order_note( __( 'IPN subscription failing payment method changed.', 'skyhs-hosting-solution' ), $subscription, $transaction_details );
							}
						}
						...
						remove_action( 'skyhshoso_subscription_activated_paypal', 'SkyHSHOSO_PayPal_Status_Manager::reactivate_subscription' );
						...
						$this->add_order_note( __( 'IPN subscription payment completed.', 'skyhs-hosting-solution' ), $transaction_order, $transaction_details );

						add_action( 'skyhshoso_subscription_activated_paypal', 'SkyHSHOSO_PayPal_Status_Manager::reactivate_subscription' );

						skyhshoso_set_paypal_id( $transaction_order, $transaction_details['subscr_id'] );
					}
					...
					$this->add_order_note( sprintf( _x( 'IPN subscription payment %s.', 'used in order note', 'skyhs-hosting-solution' ), $transaction_details['payment_status'] ), $subscription, $transaction_details );

					if ( ! $is_first_payment ) {

						$transaction_order->update_meta_data( '_transaction_id', $transaction_details['txn_id'] );
						$transaction_order->save();

						if ( 'failed' == strtolower( $transaction_details['payment_status'] ) ) {
							$subscription->payment_failed();
							// translators: placeholder is payment status (e.g. "completed")
							$this->add_order_note( sprintf( _x( 'IPN subscription payment %s.', 'used in order note', 'skyhs-hosting-solution' ), $transaction_details['payment_status'] ), $transaction_order, $transaction_details );
						} else {
							$transaction_order->update_status( 'on-hold' );
							// translators: 1: payment status (e.g. "completed"), 2: pending reason.
							$this->add_order_note( sprintf( _x( 'IPN subscription payment %1$s for reason: %2$s.', 'used in order note', 'skyhs-hosting-solution' ), $transaction_details['payment_status'], $transaction_details['pending_reason'] ), $transaction_order, $transaction_details );
						}
					}
			...
			case 'recurring_payment_suspended':
				...
				if ( skyhshoso_get_paypal_id( $subscription ) != $ipn_profile_id ) {
					...
					$subscription->update_status( 'on-hold', __( 'IPN subscription suspended.', 'skyhs-hosting-solution' ) );
			...
			case 'subscr_cancel':
				...
				if ( skyhshoso_get_paypal_id( $subscription ) != $transaction_details['subscr_id'] ) {
					...
					$subscription->cancel_order( __( 'IPN subscription cancelled.', 'skyhs-hosting-solution' ) );
			...
			case 'recurring_payment_suspended_due_to_max_failed_payment': // Recurring payment failed
				...
				$ipn_failure_note = __( 'IPN subscription payment failure.', 'skyhs-hosting-solution' );
		...
		if ( empty( $order_id ) && isset( $args['custom'] ) ) {
			...
			if ( is_object( $order_details ) ) { // WC 2.3.11+ converted the custom value to JSON, if we have an object, we've got valid JSON
				...
				if ( 'shop_order' === $order_type ) {
					...
					// Subscription created with Subscriptions < 2.0
					$subscriptions = skyhshoso_get_subscriptions_for_order( absint( $order_details->order_id ), array( 'order_type' => array( 'parent' ) ) );
				...
				WC_Gateway_Paypal::log( __( 'Invalid PayPal IPN Payload: unable to find matching subscription.', 'skyhs-hosting-solution' ) );
			}
		}
	...
	protected static function cancel_subscription( $subscription, $old_paypal_subscriber_id ) {
		...
		if ( skyhshoso_is_paypal_profile_a( $old_paypal_subscriber_id, 'billing_agreement' ) ) {
			return;
		}

		$current_profile_id = skyhshoso_get_paypal_id( $subscription->get_id() );

		// Update the subscription using the old profile ID
		skyhshoso_set_paypal_id( $subscription, $old_paypal_subscriber_id );

		// Call update_subscription_status() directly as we don't want the notes added by SkyHSHOSO_PayPal_Status_Manager::cancel_subscription()
		SkyHSHOSO_PayPal_Status_Manager::update_subscription_status( $subscription, 'Cancel' );

		// Restore the current profile ID
		skyhshoso_set_paypal_id( $subscription, $current_profile_id );
	}
	...
	protected function add_order_note( $note, $order, $transaction_details ) {
		$note = apply_filters( 'skyhshoso_paypal_ipn_note', $note, $order, $transaction_details );
		if ( ! empty( $note ) ) {
