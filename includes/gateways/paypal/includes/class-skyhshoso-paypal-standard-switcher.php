<?php
/**
 * SkyHS Hosting Solution PayPal Subscription Switcher Class.
 *
...
class SkyHSHOSO_PayPal_Standard_Switcher {
	...
	public static function can_item_be_switched( $item_can_be_switch, $item, $subscription ) {
		...
		if ( false === $item_can_be_switch && 'paypal' === $subscription->get_payment_method() && SkyHSHOSO_PayPal::are_reference_transactions_enabled() ) {

			$is_billing_agreement = skyhshoso_is_paypal_profile_a( skyhshoso_get_paypal_id( $subscription->get_id() ), 'billing_agreement' );

			if ( 'line_item' == $item['type'] && skyhshoso_is_product_switchable_type( $item['product_id'] ) ) {
	...
	public static function cart_needs_payment( $needs_payment, $cart ) {
		...
		$cart_switch_items = skyhshoso_cart_contains_switches();

		if ( false === $needs_payment && 0 == $cart->total && false !== $cart_switch_items && false ) { // skyhshoso_is_manual_renewal_required() replaced with false

			foreach ( $cart_switch_items as $cart_switch_details ) {

				$subscription = skyhshoso_get_subscription( $cart_switch_details['subscription_id'] );

				if ( 'paypal' === $subscription->get_payment_method() && ! skyhshoso_is_paypal_profile_a( skyhshoso_get_paypal_id( $subscription->get_id() ), 'billing_agreement' ) ) {
					$needs_payment = true;
					break;
				}
			}
	...
	public static function maybe_set_payment_method( $payment_processing_result, $order_id ) {
		...
		if ( skyhshoso_order_contains_switch( $order_id ) ) {
			...
			foreach ( skyhshoso_get_subscriptions_for_switch_order( $order_id ) as $subscription ) {

				$order_payment_method = $order->get_payment_method();

				if ( 'paypal' === $subscription->get_payment_method() && $subscription->get_payment_method() !== $order_payment_method && false === skyhshoso_is_paypal_profile_a( skyhshoso_get_paypal_id( $subscription->get_id() ), 'billing_agreement' ) ) {
	...
	public static function save_old_paypal_meta( $order_id, $posted ) {
		...
		if ( ! skyhshoso_is_order( $order ) || ! skyhshoso_order_contains_switch( $order ) ) {
			return;
		}

		$subscriptions = skyhshoso_get_subscriptions_for_order( $order, array( 'order_type' => 'switch' ) );

		foreach ( $subscriptions as $subscription ) {

			if ( 'paypal' === $subscription->get_payment_method() ) {

				$paypal_id = skyhshoso_get_paypal_id( $subscription->get_id() );

				if ( ! skyhshoso_is_paypal_profile_a( $paypal_id, 'billing_agreement' ) ) {
		...
		if ( 'paypal_standard' === $order->get_meta( '_old_payment_method' )  ) {
			...
			if ( ! empty( $old_profile_id ) ) {
				...
				foreach ( $subscriptions as $subscription ) {
					...
					if ( ! skyhshoso_is_paypal_profile_a( $old_profile_id, 'billing_agreement' ) ) {
						...
						SkyHSHOSO_PayPal_Status_Manager::suspend_subscription( $subscription );
	...
	public static function get_available_payment_gateways( $available_gateways ) {
		...
		if ( ! is_wc_endpoint_url( 'order-pay' ) && ( skyhshoso_cart_contains_switches() || ( $order_id && skyhshoso_order_contains_switch( $order_id ) ) ) ) {
			foreach ( $available_gateways as $gateway_id => $gateway ) {
				if ( 'paypal' == $gateway_id && false == SkyHSHOSO_PayPal::are_reference_transactions_enabled() ) {
	...
	public static function maybe_cancel_paypal_after_switch( $order_id, $old_status, $new_status ) {
		...
		wc_doing_it_wrong( __METHOD__, 'Deprecated since 2.1', 'SkyHS Hosting Solution' );
		...
		if ( $order_completed && skyhshoso_order_contains_switch( $order_id ) ) {
	 ...
	 *
	 * Hooked onto 'wc_subscriptions_paypal_standard_suspension_note'. @see SkyHSHOSO_PayPal_Standard_Switcher::cancel_paypal_standard_after_switch()
	 *
	 * @since 1.0.0
	...
	public static function filter_suspended_switch_note() {
		return __( 'Subscription changed from PayPal Standard to PayPal Reference Transactions via customer initiated switch. The PayPal Standard subscription has been suspended.', 'skyhs-hosting-solution' );
	}
}
