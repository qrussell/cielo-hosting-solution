<?php
/**
 * SkyHS Hosting Solution PayPal Reference Transaction API Response Class
 *
...
class SkyHSHOSO_PayPal_Reference_Transaction_API_Response extends WC_Gateway_Paypal_Response {
	...
	public function get_api_error_message() {
		...
		foreach ( range( 0, 9 ) as $index ) {
			...
			if ( $this->has_parameter( "L_SHORTMESSAGE{$index}" ) ) {
				...
				$error_message = sprintf( '%s: %s - %s',
					$this->has_parameter( "L_SEVERITYCODE{$index}" ) ? $this->get_parameter( "L_SEVERITYCODE{$index}" ) : _x( 'Error', 'used in api error message if there is no severity code from PayPal', 'skyhs-hosting-solution' ),
					$this->get_parameter( "L_SHORTMESSAGE{$index}" ),
					$this->has_parameter( "L_LONGMESSAGE{$index}" ) ? $this->get_parameter( "L_LONGMESSAGE{$index}" ) : _x( 'Unknown error', 'used in api error message if there is no long message', 'skyhs-hosting-solution' )
				);
		...
		return empty( $error_messages ) ? _x( 'N/A', 'no information about something', 'skyhs-hosting-solution' ) : trim( implode( ', ', $error_messages ) );
	}
