<?php
/**
 * SkyHS Hosting Solution PayPal Reference Transaction API Class
 *
...
class SkyHSHOSO_PayPal_Reference_Transaction_API extends SkyHSHOSO_SV_API_Base {
	...
	public function are_reference_transactions_enabled() {
		...
		$this->set_response_handler( 'SkyHSHOSO_PayPal_Reference_Transaction_API_Response' );
	 ...
	 *
	 * @param array $args @see SkyHSHOSO_PayPal_Reference_Transaction_API_Request::set_express_checkout() for details
	 * @throws Exception network timeouts, etc
	 * @return SkyHSHOSO_PayPal_Reference_Transaction_API_Response_Checkout response object
	 * @since 1.0.0
	 */
	public function set_express_checkout( $args ) {
		...
		$this->set_response_handler( 'SkyHSHOSO_PayPal_Reference_Transaction_API_Response_Checkout' );
	 ...
	 * @return SkyHSHOSO_PayPal_Reference_Transaction_API_Response_Billing_Agreement response object
	 * @since 1.0.0
	 */
	public function create_billing_agreement( $token ) {
		...
		$this->set_response_handler( 'SkyHSHOSO_PayPal_Reference_Transaction_API_Response_Billing_Agreement' );
	 ...
	 * @param array $args
	 * @return SkyHSHOSO_PayPal_Reference_Transaction_API_Response_Payment refund response
	 * @since 1.0.0
	 */
	public function do_express_checkout( $token, $order, $args ) {
		...
		$this->set_response_handler( 'SkyHSHOSO_PayPal_Reference_Transaction_API_Response_Payment' );
	...
	public function do_reference_transaction( $reference_id, $order, $args ) {
		...
		$this->set_response_handler( 'SkyHSHOSO_PayPal_Reference_Transaction_API_Response_Recurring_Payment' );
	...
	public function get_callback_url( $action ) {
		return esc_url(
			add_query_arg(
				'action',
				$action,
				WC()->api_request_url( 'skyhshoso_paypal' )
			),
			null,
			'db'
		);
	 ...
	 *
	 * @see \SkyHSHOSO_SV_API_Base::get_new_request()
	 * @param array $args
	 * @return SkyHSHOSO_PayPal_Reference_Transaction_API_Request API request object
	 * @since 1.0.0
	 */
	protected function get_new_request( $args = array() ) {
		return new SkyHSHOSO_PayPal_Reference_Transaction_API_Request( $this->api_username, $this->api_password, $this->api_signature, self::VERSION );
	}

	/**
	 * Supposed to return the main gatewya plugin class, but we don't have one of those
	 *
	 * @see \SkyHSHOSO_SV_API_Base::get_plugin()
	 * @return object
	 * @since 1.0.0
	 */
	protected function get_plugin() {
		return SkyHSHOSO_PayPal::instance();
	}
}
