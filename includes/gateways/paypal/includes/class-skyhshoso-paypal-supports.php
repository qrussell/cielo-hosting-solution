<?php
/**
 * SkyHS Hosting Solution PayPal Subscription Support Class.
 *
...
class SkyHSHOSO_PayPal_Supports {
	...
	public static function add_feature_support_for_gateway( $is_supported, $feature, $gateway ) {
		...
		if ( 'paypal' === $gateway->id && SkyHSHOSO_PayPal::are_credentials_set() ) {

			if ( in_array( $feature, self::$standard_supported_features ) ) {
				$is_supported = true;
			} elseif ( in_array( $feature, self::$reference_transaction_supported_features ) && SkyHSHOSO_PayPal::are_reference_transactions_enabled() ) {
				$is_supported = true;
			}
		}
	...
	public static function add_feature_support_for_subscription( $is_supported, $feature, $subscription ) {
		...
		if ( 'paypal' === $subscription->get_payment_method() && SkyHSHOSO_PayPal::are_credentials_set() ) {

			$paypal_profile_id    = skyhshoso_get_paypal_id( $subscription->get_id() );
			$is_billing_agreement = skyhshoso_is_paypal_profile_a( $paypal_profile_id, 'billing_agreement' );
			...
			if ( 'gateway_scheduled_payments' === $feature && $is_billing_agreement ) {
				...
				if ( skyhshoso_is_paypal_profile_a( $paypal_profile_id, 'out_of_date_id' ) ) {
					$is_supported = false;
				} else {
					$is_supported = true;
				}
