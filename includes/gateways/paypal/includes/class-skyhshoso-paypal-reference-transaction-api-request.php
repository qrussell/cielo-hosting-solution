<?php
/**
 * SkyHS Hosting Solution PayPal Reference Transaction API Request Class
 *
...
class SkyHSHOSO_PayPal_Reference_Transaction_API_Request {
	...
	public function set_express_checkout( $args ) {
		...
		$default_description = sprintf( _x( 'Orders with %s', 'data sent to paypal', 'skyhs-hosting-solution' ), get_bloginfo( 'name' ) );
		...
		$this->add_parameters( array(
			...
			'L_BILLINGAGREEMENTDESCRIPTION0' => skyhshoso_get_paypal_item_name( $args['billing_description'] ),
	...
	public function do_reference_transaction( $reference_id, $order, $args = array() ) {
		...
		$defaults = array(
			...
			'invoice_number'       => SkyHSHOSO_PayPal::get_option( 'invoice_prefix' ) . sanitize_title( ltrim( $order->get_order_number(), _x( '#', 'hash before the order number. Used as a character to remove from the actual order number', 'skyhs-hosting-solution' ) ) ),
	...
	protected function add_payment_details_parameters( WC_Order $order, $type, $use_deprecated_params = false ) {
		...
		foreach ( $order->get_items() as $item ) {
			...
			$order_items[] = array(
				'NAME'    => skyhshoso_get_paypal_item_name( $product->get_title() ),
		...
		foreach ( $order->get_fees() as $fee ) {
			...
			$order_items[] = array(
				'NAME' => skyhshoso_get_paypal_item_name( $fee['name'] ),
		...
		if ( $order->get_total_discount() > 0 ) {
			...
			$order_items[] = array(
				'NAME' => __( 'Total Discount', 'skyhs-hosting-solution' ),
				'QTY'  => 1,
		...
		if ( $this->skip_line_items( $order, $order_items ) ) {
			...
			$this->add_line_item_parameters( array(
				// translators: placeholder is blogname
				'NAME' => sprintf( __( '%s - Order', 'skyhs-hosting-solution' ), get_option( 'blogname' ) ),
				'DESC' => skyhshoso_get_paypal_item_name( implode( ', ', $item_names ) ),
			...
			if ( $use_deprecated_params ) {
				$this->add_parameters( array(
					...
					'INVNUM'           => SkyHSHOSO_PayPal::get_option( 'invoice_prefix' ) . sanitize_title( ltrim( $order->get_order_number(), _x( '#', 'hash before the order number. Used as a character to remove from the actual order number', 'skyhs-hosting-solution' ) ) ),
					'PAYMENTACTION'    => $type,
	...
	public function manage_recurring_payments_profile_status( $profile_id, $new_status, $order = null ) {
		...
		$this->add_parameters( apply_filters( 'woocommerce_subscriptions_paypal_change_status_data', array_merge( $this->get_parameters(), array(
			...
			'NOTE'      => html_entity_decode( sprintf( _x( '%1$s subscription event triggered at %2$s', 'data sent to paypal', 'skyhs-hosting-solution' ), $new_status, get_bloginfo( 'name' ) ), ENT_NOQUOTES, 'UTF-8' ),
	...
	private function get_item_description( $item, $product ) {
		...
		if ( ! empty( WC()->cart ) && is_array( $item ) && empty( $item['item_meta'] ) ) {
			...
			if ( ! empty( $item_desc ) ) {
				...
				$item_desc = is_callable( array( $product, 'get_sku' ) ) && $product->get_sku() ? sprintf( __( 'SKU: %s', 'skyhs-hosting-solution' ), $product->get_sku() ) : null;
			}
		}

		return skyhshoso_get_paypal_item_name( $item_desc );
	}
	...
	public function get_parameters() {
		 ...
		 * @param \SkyHSHOSO_PayPal_Reference_Transaction_API_Request $this instance
		 */
		$this->parameters = apply_filters( 'skyhshoso_paypal_request_params', $this->parameters, $this );
		...
		foreach ( $this->parameters as $key => $value ) {
			...
			if ( false !== strpos( $key, 'AMT' ) ) {
				...
				if ( isset( $this->parameters['PAYMENTREQUEST_0_CURRENCYCODE'] ) && 'USD' == $this->parameters['PAYMENTREQUEST_0_CURRENCYCODE'] && $value > 10000 ) {
					throw new Exception( sprintf( '%s amount of %s must be less than $10,000.00', esc_html( $key ), esc_html( wp_strip_all_tags( wc_price( $value ) ) ) ) );
				}
	...
	private function skip_line_items( $order = null, $order_items = null ) {
		...
		$skip_line_items = $order->get_prices_include_tax();
		 ...
		 */
		return apply_filters( 'skyhshoso_paypal_reference_transaction_skip_line_items', $skip_line_items, $order );
	}
