<?php
/**
 * SkyHS Hosting Solution PayPal Standard Change Subscription Payment Method Class.
 *
...
class SkyHSHOSO_PayPal_Standard_Change_Payment_Method {
	...
	public static function init() {
		...
		add_action( 'skyhshoso_pre_update_payment_method', __CLASS__ . '::maybe_remove_subscription_cancelled_callback', 10, 3 );
		add_action( 'skyhshoso_subscription_payment_method_updated', __CLASS__ . '::maybe_reattach_subscription_cancelled_callback', 10, 3 );

		// Don't update payment methods immediately when changing to PayPal - wait for the IPN notification
		add_filter( 'skyhshoso_subscriptions_update_payment_via_pay_shortcode', __CLASS__ . '::maybe_dont_update_payment_method', 10, 3 );

		add_filter( 'skyhshoso_gateway_change_payment_button_text', __CLASS__ . '::change_payment_button_text', 10, 2 );
	}
	...
	public static function maybe_remove_subscription_cancelled_callback( $subscription, $new_payment_method, $old_payment_method ) {
		if ( 'paypal' == $new_payment_method && 'paypal' == $old_payment_method && ! SkyHSHOSO_PayPal::are_reference_transactions_enabled() ) {
			remove_action( 'skyhshoso_subscription_cancelled_paypal', 'SkyHSHOSO_PayPal_Status_Manager::cancel_subscription' );
		}
	}
	...
	public static function maybe_reattach_subscription_cancelled_callback( $subscription, $new_payment_method, $old_payment_method ) {
		if ( 'paypal' == $new_payment_method && 'paypal' == $old_payment_method && ! SkyHSHOSO_PayPal::are_reference_transactions_enabled() ) {
			add_action( 'skyhshoso_subscription_cancelled_paypal', 'SkyHSHOSO_PayPal_Status_Manager::cancel_subscription' );
		}
	}
