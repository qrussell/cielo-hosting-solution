<?php
/**
 * SkyHS Hosting Solution PayPal Reference Transaction API Do Express Checkout Response Class
 *
...
class SkyHSHOSO_PayPal_Reference_Transaction_API_Response_Payment extends SkyHSHOSO_PayPal_Reference_Transaction_API_Response_Billing_Agreement {
	...
	public function get_status_message() {
		...
		if ( $this->transaction_held() ) {
			...
			$message = sprintf( __( 'expected clearing date %s', 'skyhs-hosting-solution' ), date_i18n( wc_date_format(), skyhshoso_date_to_time( $this->get_payment_parameter( 'EXPECTEDECHECKCLEARDATE' ) ) ) );
		}
	 ...
	 *
	 * A wrapper around @see SkyHSHOSO_PayPal_Reference_Transaction_API_Response::has_parameter()
	 ...
	 *
	 * A wrapper around @see SkyHSHOSO_PayPal_Reference_Transaction_API_Response::get_parameter()
	 * that prepends the @see self::get_payment_parameter_prefix().
	 *
	 * @since 1.0.0
	 * @param string $name parameter name
