<?php
/**
 * SkyHS Hosting Solution PayPal Subscription Class.
 *
...
class SkyHSHOSO_PayPal {

	/** @var SkyHSHOSO_PayPal_Express_API for communicating with PayPal */
	protected static $api;

	/** @var SkyHSHOSO_PayPal single instance of this class */
	...
	public static function init() {
		...
		if ( ! has_action( 'woocommerce_api_skyhshoso_paypal' ) ) {
			add_action( 'woocommerce_api_skyhshoso_paypal', __CLASS__ . '::handle_wc_api' );
		}
		...
		add_filter( 'skyhshoso_resubscribe_order_created', __CLASS__ . '::remove_resubscribe_order_meta', 10, 2 );

		// Triggered by SkyHSHOSO_SV_API_Base::broadcast_request() whenever an API request is made
		...
		add_action( 'valid-paypal-standard-ipn-request', 'SkyHSHOSO_PayPal_Standard_IPN_Failure_Handler::attach', -1, 1 );
		add_action( 'valid-paypal-standard-ipn-request', 'SkyHSHOSO_PayPal_Standard_IPN_Failure_Handler::detach', 1, 1 );
		...
		SkyHSHOSO_PayPal_Supports::init();
		SkyHSHOSO_PayPal_Status_Manager::init();
		SkyHSHOSO_PayPal_Standard_Switcher::init();
		SkyHSHOSO_PayPal_Standard_Change_Payment_Method::init();

		if ( is_admin() ) {
			SkyHSHOSO_PayPal_Admin::init();
		}
	}
	...
	public static function get_option( $setting_key ) {
		...
		if ( version_compare( WC()->version, '3.3', '<' ) && in_array( $setting_key, array( 'api_username', 'api_password', 'api_signature' ) ) && 'yes' === self::get_option( 'testmode' ) ) {
	...
	public static function are_reference_transactions_enabled( $bypass_cache = '' ) {
		...
		$transient_key                  = 'skyhshoso_paypal_rt_enabled';
		$reference_transactions_enabled = false;

		if ( self::are_credentials_set() ) {

			$accounts_with_reference_transactions_enabled = json_decode( get_option( 'skyhshoso_paypal_rt_enabled_accounts', wp_json_encode( array() ) ) );
			...
			if ( in_array( $api_username, $accounts_with_reference_transactions_enabled ) ) {
				...
				if ( self::get_api()->are_reference_transactions_enabled() ) {
					...
					update_option( 'skyhshoso_paypal_rt_enabled_accounts', wp_json_encode( $accounts_with_reference_transactions_enabled ) );
					$reference_transactions_enabled = true;
				} else {
			...
			case 'create_billing_agreement':
				...
				try {
					...
					if ( 1 == $express_checkout_details_response->get_billing_agreement_status() ) {
						...
						if ( is_null( $order ) ) {
							throw new Exception( __( 'Unable to find order for PayPal billing agreement.', 'skyhs-hosting-solution' ) );
						}

						// we need to process an initial payment
						if ( $order->get_total() > 0 && false ) { // SkyHS doesn't use WC Subscription post type
						...
						if ( false ) { // skyhshoso_is_subscription replaced with false
							// Payment method change not supported via WC_Subscriptions_Change_Payment_Gateway in SkyHS
						...
						skyhshoso_set_paypal_id( $order, $billing_agreement_response->get_billing_agreement_id() );

						// Update payment method on all active subscriptions?
						if ( false ) { // skyhshoso_is_subscription replaced with false
							// Payment method update not supported via WC_Subscriptions_Change_Payment_Gateway in SkyHS
						}

						// SkyHS: iterate over subscriptions linked to this order
						$skyhso_subscriptions = SkyHSHOSO_Subscription_DB::get_by_order( $order->get_id() );
						if ( ! empty( $skyhso_subscriptions ) ) {
							foreach ( $skyhso_subscriptions as $skyhso_subscription ) {
								if ( is_object( $skyhso_subscription ) && method_exists( $skyhso_subscription, 'set_payment_method' ) ) {
									$skyhso_subscription->set_payment_method( $payment_method );
								}
								skyhshoso_set_paypal_id( $skyhso_subscription, $billing_agreement_response->get_billing_agreement_id() );
							}
						}
						...
						if ( isset( $redirect_url ) ) {
							...
							wp_safe_redirect( esc_url_raw( add_query_arg( 'utm_nooverride', '1', $order->get_checkout_order_received_url() ) ) );
						}
					...
					wc_add_notice( __( 'An error occurred, please try again or try an alternate form of payment.', 'skyhs-hosting-solution' ), 'error' );
	...
	public static function get_paypal_args( $paypal_args, $order ) {
		...
		if ( self::order_contains_subscription( $order ) || false ) { // second condition was skyhshoso_is_subscription
			if ( self::are_reference_transactions_enabled() ) {
				$paypal_args = self::get_api()->get_paypal_args( $paypal_args, $order );
			} else {
				$paypal_args = SkyHSHOSO_PayPal_Standard_Request::get_paypal_args( $paypal_args, $order );
			}
		}
	...
	/**
	 * Check if an order contains a SkyHS subscription.
	 *
	 * @since 1.0.0
	 */
	protected static function order_contains_subscription( $order ) {
		if ( ! is_object( $order ) ) {
		...
		return get_post_meta( $order_id, '_skyhshoso_renewal_subscription_id', true ) || get_post_meta( $order_id, '_skyhshoso_subscriptions_created', true );
	}
	...
	public static function process_ipn_request( $transaction_details ) {
		...
		try {
			...
			SkyHSHOSO_PayPal_Standard_IPN_Failure_Handler::log_unexpected_exception( $e );
		}
	}
	...
	public static function process_subscription_payment( $amount, $order ) {
		...
		$paypal_profile_id = skyhshoso_get_paypal_id( $order->get_id() );

		if ( skyhshoso_is_paypal_profile_a( $paypal_profile_id, 'billing_agreement' ) ) {
			...
			$response = self::get_api()->do_reference_transaction( $paypal_profile_id, $order, array(
				'amount'         => $amount,
				'invoice_number' => self::get_option( 'invoice_prefix' ) . sanitize_title( ltrim( $order->get_order_number(), _x( '#', 'hash before the order number. Used as a character to remove from the actual order number', 'skyhs-hosting-solution' ) ) ),
			) );
	...
	public static function process_subscription_payment_response( $order, $response ) {
		...
		if ( $response->has_api_error() ) {
			...
			$order->update_status( 'failed', sprintf( __( 'PayPal API error: (%1$d) %2$s', 'skyhs-hosting-solution' ), $response->get_api_error_code(), $error_message ) );

		} elseif ( $response->transaction_held() ) {

			// translators: placeholder is PayPal transaction status message
			$order_note   = sprintf( __( 'PayPal Transaction Held: %s', 'skyhs-hosting-solution' ), $response->get_status_message() );
			$order_status = apply_filters( 'skyhshoso_paypal_held_payment_order_status', 'on-hold', $order, $response );
			...
			// translators: placeholder is PayPal transaction status message
			$order->update_status( 'failed', sprintf( __( 'PayPal payment declined: %s', 'skyhs-hosting-solution' ), $response->get_status_message() ) );

		} elseif ( $response->transaction_approved() ) {
			// translators: placeholder is a transaction ID.
			$order->add_order_note( sprintf( __( 'PayPal payment approved (ID: %s)', 'skyhs-hosting-solution' ), $response->get_transaction_id() ) );
	...
	public static function remove_resubscribe_order_meta( $resubscribe_order, $subscription ) {
		...
		foreach ( $post_meta_keys as $post_meta_key ) {
			delete_post_meta( $resubscribe_order->get_id(), $post_meta_key );
		}
	...
	public static function maybe_add_change_payment_method_warning( $script_parameters ) {
		...
		if ( class_exists( '\Automattic\WooCommerce\Utilities\FeaturesUtil' ) && \Automattic\WooCommerce\Utilities\FeaturesUtil::feature_is_enabled( 'custom_order_tables' ) ) {
			...
			$subscription = skyhshoso_get_subscription( $post->ID );
		}
		...
		$is_paypal_standard = ! skyhshoso_is_paypal_profile_a( $paypal_profile_id, 'billing_agreement' );

		if ( $is_paypal_standard ) {
			$script_parameters['change_payment_method_warning'] = __( "Are you sure you want to change the payment method from PayPal standard?\n\nThis will suspend the subscription at PayPal.", 'skyhs-hosting-solution' );
		}
	...
	public static function maybe_override_needs_payment( $needs_payment, $order ) {
		if ( $needs_payment && self::instance()->get_id() === $order->get_payment_method() && ! self::are_reference_transactions_enabled() && self::order_contains_subscription( $order ) ) {
			$has_lock            = $order->get_meta( '_skyhshoso_lock_order_payment' );
			$seconds_since_order = skyhshoso_seconds_since_order_created( $order );

			// We have lock and order hasn't meet the lock time.
			if ( $has_lock && $seconds_since_order < apply_filters( 'skyhshoso_lock_order_payment_seconds', 180 ) ) {
				$needs_payment = false;
			}
		}
	...
	public static function maybe_add_payment_lock() {
		...
		if ( $order && self::instance()->get_id() === $order->get_payment_method() && $order->needs_payment() && ! self::are_reference_transactions_enabled() && self::order_contains_subscription( $order ) ) {
			$order->update_meta_data( '_skyhshoso_lock_order_payment', 'true' );
			$order->save();
		}
	}
	...
	public static function maybe_remove_payment_lock( $order_id ) {
		...
		if ( self::instance()->get_id() === $order->get_payment_method() && self::order_contains_subscription( $order ) ) {
			$order->delete_meta_data( 'skyhshoso_lock_order_payment' );
			$order->save();
		}
	}
	 ...
	 * Get the API object
	 *
	 * @return SkyHSHOSO_PayPal_Express_API API instance
	 * @since 1.0.0
	 */
	protected static function get_ipn_handler( $ipn_type = 'standard' ) {
		...
		if ( 'reference' === $ipn_type ) {
			...
			if ( ! isset( self::$ipn_handlers['reference'] ) ) {
				self::$ipn_handlers['reference'] = new SkyHSHOSO_PayPal_Reference_Transaction_IPN_Handler( $use_sandbox, self::get_option( 'receiver_email' ) );
			}
			...
			if ( ! isset( self::$ipn_handlers['standard'] ) ) {
				self::$ipn_handlers['standard'] = new SkyHSHOSO_PayPal_Standard_IPN_Handler( $use_sandbox, self::get_option( 'receiver_email' ) );
			}
	...
	public static function get_api() {
		...
		return self::$api = new SkyHSHOSO_PayPal_Reference_Transaction_API( 'paypal', $environment, self::get_option( 'api_username' ), self::get_option( 'api_password' ), self::get_option( 'api_signature' ) );
	}
	...
	/** Method required by SkyHSHOSO_SV_API_Base, which normally requires an instance of SV_WC_Plugin **/

	public function get_plugin_name() {
		return _x( 'SkyHS Hosting Solution PayPal', 'used in User Agent data sent to PayPal to help identify where a payment came from', 'skyhs-hosting-solution' );
	}

	public function get_version() {
		return SKYHSHOSO_VERSION;
	}
	...
	public static function set_enabled_for_subscriptions_default() {
		...
		if ( 'yes' === SkyHSHOSO_PayPal::get_option( 'enabled' ) && skyhshoso_do_subscriptions_exist() ) {
			$default = 'yes';
		} else {
			$default = 'no';
		}
		...
		foreach ( WC()->payment_gateways->payment_gateways as $gateway ) {
			if ( $gateway->id === 'paypal' ) {
				skyhshoso_update_settings_option( $gateway, 'enabled_for_subscriptions', $default );
				break;
			}
		}
	}
	...
	public static function maybe_remove_paypal_standard( $available_gateways ) {
		...
		if ( ! isset( $available_gateways['paypal'] ) || 'yes' === self::get_option( 'enabled_for_subscriptions' ) || SkyHSHOSO_PayPal::are_reference_transactions_enabled() ) {
		...
		if (
			SkyHSHOSO_Change_Payment_Gateway::$is_request_to_change_payment ||
			SkyHSHOSO_Payment_Gateways::instance()->cart_contains_subscription() ||
			$cart_contains_renewal ||
			( $paying_for_order && self::order_contains_subscription( $paying_for_order ) )
		) {
	 ...
	 * @param string $return    Optional. The type to return. Can be 'ids' to return subscription IDs or 'objects' to return subscription objects. Default 'ids'.
	 * @return array Subscriptions (objects or IDs) with the PayPal Profile ID or Billing Agreement stored in meta.
	 */
	public static function get_subscriptions_by_paypal_id( $paypal_id, $return = 'ids' ) {
		...
		if ( 'objects' === $return ) {
			$subscriptions = array_filter( array_map( 'skyhshoso_get_subscription', self::$subscriptions_by_paypal_id[ $paypal_id ] ) );
		} else {
