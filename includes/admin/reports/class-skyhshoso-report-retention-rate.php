 ...
 * @since      2.1
 */
class SkyHSHOSO_Report_Retention_Rate extends WC_Admin_Report {
	...
	private function query_report_data() {
		...
		$days_in_interval_period = skyhshoso_get_days_in_cycle( $this->report_data->interval_period, 1 );
		...
		$base_query = $wpdb->prepare(
			...
			skyhshoso_get_date_meta_key( 'cancelled' ), // If a subscription has a cancelled date, use that to determine a more accurate lifetime
			skyhshoso_get_date_meta_key( 'end' ) // Otherwise, we want to use the end date for subscritions that have expired
		);
	...
	public function output_report() {
		include( SKYHSHOSO_PLUGIN_DIR . 'includes/admin/views/html-report-by-period.php' );
	}
	...
	public function get_main_chart() {
		...
		switch ( $this->report_data->interval_period ) {
			case 'day':
				$x_axes_label = _x( 'Number of days after sign-up', 'X axis label on retention rate graph', 'skyhs-hosting-solution' );
				break;
			case 'week':
				$x_axes_label = _x( 'Number of weeks after sign-up', 'X axis label on retention rate graph', 'skyhs-hosting-solution' );
				break;
			case 'month':
				$x_axes_label = _x( 'Number of months after sign-up', 'X axis label on retention rate graph', 'skyhs-hosting-solution' );
				break;
		}

		?>
		<div class="chart-container" id="skyhs_hosting_solution_retention_chart">
					...
					main_chart = jQuery.plot(
						...
						series,
						{
							...
							yaxes: [ {
								...
								tickDecimals: 0,
								color: '#d4d9dc',
								axisLabel: "<?php echo esc_js( __( 'Unended Subscription Count', 'skyhs-hosting-solution' ) ); ?>",
								axisLabelPadding: 18,
								font: {
									color: "#aaa"
								}
