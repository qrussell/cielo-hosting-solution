...
class SkyHSHOSO_Report_Dashboard {
	...
	public static function get_data( $args = array() ) {
		...
		$args         = apply_filters( 'skyhshoso_reports_subscription_dashboard_args', $args );
		$args         = wp_parse_args( $args, $default_args );
		$offset       = get_option( 'gmt_offset' );
		$update_cache = false;

		// Use this once it is merged - skyhshoso_get_gmt_offset_string();
		...
		// phpcs:disable WordPress.DB.PreparedSQLPlaceholders.QuotedDynamicPlaceholder
		...
		// phpcs:enable WordPress.DB.PreparedSQLPlaceholders.QuotedDynamicPlaceholder
		...
		// phpcs:disable WordPress.DB.PreparedSQLPlaceholders.QuotedDynamicPlaceholder, WordPress.DB.PreparedSQL.InterpolatedNotPrepared
		...
		// phpcs:enable WordPress.DB.PreparedSQLPlaceholders.QuotedDynamicPlaceholder, WordPress.DB.PreparedSQL.InterpolatedNotPrepared
	...
	public static function add_stats_to_dashboard() {
		...
		<li class="signup-count">
			<a href="<?php echo esc_url( admin_url( 'admin.php?page=wc-reports&tab=subscriptions&report=subscription_events_by_date&range=month' ) ); ?>">
				...
				echo wp_kses_post( sprintf( _n( '%2$s%1$s signup%3$s subscription signups this month', '%2$s%1$s signups%3$s subscription signups this month', $report_data->signup_count, 'skyhs-hosting-solution' ), $report_data->signup_count, '<strong>', '</strong>' ) );
				?>
			</a>
		</li>
		<li class="signup-revenue">
			<a href="<?php echo esc_url( admin_url( 'admin.php?page=wc-reports&tab=subscriptions&report=subscription_events_by_date&range=month' ) ); ?>">
				...
				echo wp_kses_post( sprintf( __( '%s signup revenue this month', 'skyhs-hosting-solution' ), '<strong>' . wc_price( $report_data->signup_revenue ) . '</strong>' ) );
				?>
			</a>
		</li>
		<li class="renewal-count">
			<a href="<?php echo esc_url( admin_url( 'admin.php?page=wc-reports&tab=subscriptions&report=subscription_events_by_date&range=month' ) ); ?>">
				<?php
				// translators: 1$: count, 2$ and 3$ are opening and closing strong tags, respectively.
				echo wp_kses_post( sprintf( _n( '%2$s%1$s renewal%3$s subscription renewals this month', '%2$s%1$s renewals%3$s subscription renewals this month', $report_data->renewal_count, 'skyhs-hosting-solution' ), $report_data->renewal_count, '<strong>', '</strong>' ) );
				?>
			</a>
		</li>
		<li class="renewal-revenue">
			<a href="<?php echo esc_url( admin_url( 'admin.php?page=wc-reports&tab=subscriptions&report=subscription_events_by_date&range=month' ) ); ?>">
				<?php
				// translators: %s: formatted amount.
				echo wp_kses_post( sprintf( __( '%s renewal revenue this month', 'skyhs-hosting-solution' ), '<strong>' . wc_price( $report_data->renewal_revenue ) . '</strong>' ) );
				?>
			</a>
		</li>
		<li class="cancel-count">
			<a href="<?php echo esc_url( admin_url( 'admin.php?page=wc-reports&tab=subscriptions&report=subscription_events_by_date&range=month' ) ); ?>">
				<?php
				// translators: 1$: count, 2$ and 3$ are opening and closing strong tags, respectively.
				echo wp_kses_post( sprintf( _n( '%2$s%1$s cancellation%3$s subscription cancellations this month', '%2$s%1$s cancellations%3$s subscription cancellations this month', $report_data->cancel_count, 'skyhs-hosting-solution' ), $report_data->cancel_count, '<strong>', '</strong>' ) ); ?>
			</a>
		</li>
		<?php
	...
	public static function dashboard_scripts() {
		wp_enqueue_style( 'skyhshoso-dashboard-report', SKYHSHOSO_PLUGIN_URL . 'assets/css/dashboard.css', array(), SKYHSHOSO_VERSION );
	}
