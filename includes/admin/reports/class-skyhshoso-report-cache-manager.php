 ...
 *
 * @class    SkyHSHOSO_Cache_Manager
 * @since    2.1
 * @package  SkyHS Hosting Solution/Classes
 * @category Class
 */
...
class SkyHSHOSO_Report_Cache_Manager {
	...
	private $update_events_and_classes = array(
		'skyhs_hosting_solution_reports_schedule_cache_updates' => array( // a custom hook that can be called to schedule a full cache update, used by SkyHSHOSO_Upgrader
			0 => 'SkyHSHOSO_Report_Dashboard',
			1 => 'SkyHSHOSO_Report_Subscription_Events_By_Date',
			2 => 'SkyHSHOSO_Report_Upcoming_Recurring_Revenue',
			4 => 'SkyHSHOSO_Report_Subscription_By_Product',
			5 => 'SkyHSHOSO_Report_Subscription_By_Customer',
		),
		...
		'skyhs_hosting_solution_switch_completed' => array(
			1 => 'SkyHSHOSO_Report_Subscription_Events_By_Date',
		),
		'woocommerce_subscription_status_changed'    => array(
			0 => 'SkyHSHOSO_Report_Dashboard',
			1 => 'SkyHSHOSO_Report_Subscription_Events_By_Date', // we really only need cancelled, expired and active status here, but we'll use a more generic hook for convenience
			5 => 'SkyHSHOSO_Report_Subscription_By_Customer',
		),
	 ...
	 */
	private $cron_hook = 'skyhshoso_report_update_cache';
	...
	public function __construct() {
		...
		add_filter( 'skyhshoso_system_status', array( $this, 'add_system_status_info' ) );

		add_action( 'skyhs_hosting_solution_upgraded', array( $this, 'transfer_large_site_cache_option' ), 10, 2 );
	}
	 ...
	 *
	 * For large sites, cache updates are run only once per day to avoid overloading the DB where the queries are very resource intensive. We do this at 4am in the
	...
	public function update_cache( $report_class ) {
		 ...
		 */
		if ( ! apply_filters( 'skyhshoso_report_cache_updates_enabled', 'yes' === get_option( 'skyhs_hosting_solution_cache_updates_enabled', 'yes' ), $report_class ) ) {
			return;
		}
	...
	protected function use_large_site_cache() {
		...
		if ( null === $this->use_large_site_cache ) {
			$this->use_large_site_cache = skyhshoso_is_large_site();
		}

		return apply_filters( 'skyhshoso_report_use_large_site_cache', $this->use_large_site_cache );
	}
	...
	public function admin_notices() {
		...
		if ( in_array( $screen->id, apply_filters( 'woocommerce_reports_screen_ids', array( $wc_screen_id . '_page_wc-reports', 'dashboard' ) ) ) && isset( $_GET['tab'] ) && 'subscriptions' == $_GET['tab'] && ( ! isset( $_GET['report'] ) || in_array( $_GET['report'], array( 'subscription_events_by_date', 'upcoming_recurring_revenue', 'subscription_by_product', 'subscription_by_customer' ) ) ) && $this->use_large_site_cache() ) { // phpcs:ignore WordPress.Security.NonceVerification.Recommended
			skyhshoso_add_admin_notice( __( 'Please note: data for this report is cached. The data displayed may be out of date by up to 24 hours. The cache is updated each morning at 4am in your site\'s timezone.', 'skyhs-hosting-solution' ) );
		}
	}
	...
	public function catch_unexpected_shutdown() {
		...
		if ( $error['type'] & ( E_ERROR | E_PARSE | E_COMPILE_ERROR | E_USER_ERROR | E_RECOVERABLE_ERROR ) ) {
			$failures = get_option( 'skyhs_hosting_solution_cache_updates_failures', 0 );
			$failures++;
			update_option( 'skyhs_hosting_solution_cache_updates_failures', $failures, false );
			 ...
			 */
			if ( $failures > apply_filters( 'skyhs_hosting_solution_cache_updates_failures_threshold', 2 ) ) {
				update_option( 'skyhs_hosting_solution_cache_updates_enabled', 'no', false );
			}
		}
	}
	...
	public function add_system_status_info( $data ) {
		$cache_enabled = ( 'yes' === get_option( 'skyhs_hosting_solution_cache_updates_enabled', 'yes' ) );
		$failures      = get_option( 'skyhs_hosting_solution_cache_updates_failures', 0 );
		$new_data      = array(
			'skyhshoso_report_cache_enabled'  => array(
				'name'    => _x( 'Report Cache Enabled', 'Whether the Report Cache has been enabled', 'skyhs-hosting-solution' ),
				'label'   => 'Report Cache Enabled',
				'note'    => $cache_enabled ? __( 'Yes', 'skyhs-hosting-solution' ) : __( 'No', 'skyhs-hosting-solution' ),
				'success' => $cache_enabled,
			),
			'skyhshoso_cache_update_failures' => array(
				'name'    => __( 'Cache Update Failures', 'skyhs-hosting-solution' ),
				'label'   => 'Cache Update Failures',
				/* translators: %d refers to the number of times we have detected cache update failures */
				'note'    => sprintf( _n( '%d failures', '%d failure', $failures, 'skyhs-hosting-solution' ), $failures ),
				'success' => 0 === $failures,
			),
		);
	...
	/**
	 * Transfers the 'skyhshoso_report_use_large_site_cache' option to the new 'skyhshoso_is_large_site' option.
	 *
	 * In 3.0.7 we introduced a more general use option, 'skyhshoso_is_large_site', replacing the need for one specifically
	...
	public function transfer_large_site_cache_option( $new_version, $previous_version ) {
		...
		if ( version_compare( $previous_version, '3.0.7', '<' ) && false !== get_option( 'skyhshoso_report_use_large_site_cache' ) ) {
			update_option( 'skyhshoso_is_large_site', 'yes', false );
			delete_option( 'skyhshoso_report_use_large_site_cache' );
		}
	}
}
