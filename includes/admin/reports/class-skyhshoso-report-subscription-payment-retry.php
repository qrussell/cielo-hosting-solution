 ...
 * @since      2.1
 */
class SkyHSHOSO_Report_Subscription_Payment_Retry extends WC_Admin_Report {
	...
	private function query_report_data() {
		...
		$query_end_date   = get_gmt_from_date( date( 'Y-m-d H:i:s', skyhshoso_strtotime_dark_knight( '+1 day', $this->end_date ) ) );
		...
		$renewal_query = $wpdb->prepare(
			...
			SELECT COUNT(DISTINCT retries.retry_id) as count, MIN(retries.date_gmt) AS retry_date_gmt, MIN({$retry_date_in_local_time}) AS retry_date, SUM(meta_order_total.meta_value) AS renewal_totals
				...
				INNER JOIN {$wpdb->prefix}skyhshoso_payment_retries AS retries ON ( orders.ID = retries.order_id )
		...
		$retry_query = $wpdb->prepare(
			...
			SELECT COUNT(DISTINCT retries.retry_id) AS count, retries.status AS status, MIN(retries.date_gmt) AS retry_date_gmt, MIN({$retry_date_in_local_time}) AS retry_date
				FROM {$wpdb->prefix}skyhshoso_payment_retries AS retries
	...
	public function get_chart_legend() {
		...
		$legend[] = array(
			...
			'title'            => sprintf( __( '%s renewal revenue recovered', 'skyhs-hosting-solution' ), '<strong>' . wc_price( $data->renewal_total_amount ) . '</strong>' ),
			'placeholder'      => __( 'The total amount of revenue, including tax and shipping, recovered with the failed payment retry system for renewal orders with a failed payment.', 'skyhs-hosting-solution' ),
		...
		$legend[] = array(
			...
			'title'       => sprintf( __( '%s renewal orders', 'skyhs-hosting-solution' ), '<strong>' . $data->renewal_total_count . '</strong>' ),
			'placeholder' => __( 'The number of renewal orders which had a failed payment use the retry system.', 'skyhs-hosting-solution' ),
			'color'       => $this->chart_colours['renewal_count'],
		);

		$legend[] = array(
			// translators: %s: retry count.
			'title'            => sprintf( __( '%s retry attempts succeeded', 'skyhs-hosting-solution' ), '<strong>' . $data->retry_success_count . '</strong>' ),
			'placeholder'      => __( 'The number of renewal payment retries for this period which were able to process the payment which had previously failed one or more times.', 'skyhs-hosting-solution' ),
		...
		$legend[] = array(
			// translators: %s: retry count.
			'title'            => sprintf( __( '%s retry attempts failed', 'skyhs-hosting-solution' ), '<strong>' . $data->retry_failed_count . '</strong>' ),
			'placeholder'      => __( 'The number of renewal payment retries for this period which did not result in a successful payment.', 'skyhs-hosting-solution' ),
		...
		$legend[] = array(
			// translators: %s: retry count.
			'title'            => sprintf( __( '%s retry attempts pending', 'skyhs-hosting-solution' ), '<strong>' . $data->retry_pending_count . '</strong>' ),
			'placeholder'      => __( 'The number of renewal payment retries not yet processed.', 'skyhs-hosting-solution' ),
	...
	public function get_main_chart() {
		...
		?>
		<div id="skyhs_hosting_solution_payment_retry_chart" class="chart-container">
		...
		<script type="text/javascript">
			...
			jQuery(function(){
				...
				var drawGraph = function( highlight ) {
					var series = [
						{
							label: "<?php echo esc_js( __( 'Successful retries', 'skyhs-hosting-solution' ) ) ?>",
						...
						{
							label: "<?php echo esc_js( __( 'Failed retries', 'skyhs-hosting-solution' ) ) ?>",
						...
						{
							label: "<?php echo esc_js( __( 'Pending retries', 'skyhs-hosting-solution' ) ) ?>",
						...
						{
							label: "<?php echo esc_js( __( 'Recovered Renewal Revenue', 'skyhs-hosting-solution' ) ) ?>",
