 ...
 * @subpackage SkyHSHOSO_Admin_Reports
 * @category   Class
 * @since      2.1
 */
class SkyHSHOSO_Report_Subscription_Events_By_Date extends WC_Admin_Report {
	...
	public function get_data( $args = array() ) {
		...
		$args = apply_filters( 'skyhshoso_reports_subscription_events_args', $args );
		...
		if ( $args['no_cache'] || ! isset( $cached_results[ $query_hash ] ) ) {
			...
			$cached_results[ $query_hash ] = apply_filters( 'skyhshoso_reports_subscription_events_sign_up_data', (array) $wpdb->get_results( $query ), $args );
		...
		$query = $wpdb->prepare(
			...
			date( 'Y-m-d', $this->start_date ),
			$query_end_date,
			skyhshoso_get_date_meta_key( 'end' ),
			$site_timezone
		);
		...
		if ( $args['no_cache'] || ! isset( $cached_results[ $query_hash ] ) ) {
			// phpcs:disable WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching, WordPress.DB.PreparedSQL.NotPrepared
			$wpdb->query( 'SET SESSION SQL_BIG_SELECTS=1' );
			$cached_results[ $query_hash ] = apply_filters( 'skyhshoso_reports_subscription_events_subscriber_count_data', (array) $wpdb->get_results( $query ), $args );
			// phpcs:enable WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching, WordPress.DB.PreparedSQL.NotPrepared
			$update_cache = true;
		}
		...
		$query = $wpdb->prepare(
			...
			skyhshoso_get_date_meta_key( 'cancelled' ),
			date( 'Y-m-d', $this->start_date ),
			$query_end_date
		);
		...
		if ( $args['no_cache'] || ! isset( $cached_results[ $query_hash ] ) ) {
			// phpcs:disable WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching, WordPress.DB.PreparedSQL.NotPrepared
			$wpdb->query( 'SET SESSION SQL_BIG_SELECTS=1' );
			$cached_results[ $query_hash ] = apply_filters( 'skyhshoso_reports_subscription_events_cancel_count_data', (array) $wpdb->get_results( $query ), $args );
			// phpcs:enable WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching, WordPress.DB.PreparedSQL.NotPrepared
			$update_cache = true;
		}
		...
		if ( $args['no_cache'] || ! isset( $cached_results[ $query_hash ] ) ) {
			// phpcs:disable WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching, WordPress.DB.PreparedSQL.NotPrepared
			$wpdb->query( 'SET SESSION SQL_BIG_SELECTS=1' );
			$cached_results[ $query_hash ] = apply_filters( 'skyhshoso_reports_subscription_events_ended_count_data', (array) $wpdb->get_results( $query ), $args );
			// phpcs:enable WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching, WordPress.DB.PreparedSQL.NotPrepared
			$update_cache = true;
		}
		...
		if ( $update_cache ) {
			...
			if ( ! skyhshoso_is_woocommerce_pre( '3.7' ) ) {
	...
	public function get_chart_legend() {
		...
		$legend[] = array(
			...
			'title'            => sprintf( __( '%s signup revenue in this period', 'skyhs-hosting-solution' ), '<strong>' . wc_price( $data->signup_orders_total_amount ) . '</strong>' ),
			'placeholder'      => __( 'The sum of all subscription parent orders, including other items, fees, tax and shipping.', 'skyhs-hosting-solution' ),
		...
		$legend[] = array(
			// translators: %s: formatted total amount.
			'title'            => sprintf( __( '%s renewal revenue in this period', 'skyhs-hosting-solution' ), '<strong>' . wc_price( $data->renewal_orders_total_amount ) . '</strong>' ),
			'placeholder'      => __( 'The sum of all renewal orders including tax and shipping.', 'skyhs-hosting-solution' ),
		...
		$legend[] = array(
			// translators: %s: formatted total amount.
			'title'            => sprintf( __( '%s resubscribe revenue in this period', 'skyhs-hosting-solution' ), '<strong>' . wc_price( $data->resubscribe_orders_total_amount ) . '</strong>' ),
			'placeholder'      => __( 'The sum of all resubscribe orders including tax and shipping.', 'skyhs-hosting-solution' ),
		...
		$legend[] = array(
			// translators: %s: formatted total amount.
			'title'            => sprintf( __( '%s switch revenue in this period', 'skyhs-hosting-solution' ), '<strong>' . wc_price( $data->switch_orders_total_amount ) . '</strong>' ),
			'placeholder'      => __( 'The sum of all switch orders including tax and shipping.', 'skyhs-hosting-solution' ),
		...
		$legend[] = array(
			'title'            => sprintf(
				...
				__( '%2$s %1$s new subscriptions', 'skyhs-hosting-solution' ),
				'<strong> <span class="skyhs-hosting-solution-count count">' . $this->report_data->new_subscription_total_count . '</span> </strong> </a>',
				'<a href="' .
				esc_url(
					add_query_arg(
						array(
			...
			'placeholder'      => __( 'The number of subscriptions created during this period, either by being manually created, imported or a customer placing an order. This includes orders pending payment.', 'skyhs-hosting-solution' ),
		...
		$legend[] = array(
			'title'            => sprintf(
				// translators: 2: link opening tag, 1: subscription count and closing tag.
				__( '%2$s %1$s subscription signups', 'skyhs-hosting-solution' ),
				'<strong> <span class="skyhs-hosting-solution-count count">' . $this->report_data->signup_orders_total_count . '</strong> </a>',
				'<a href="' .
				esc_url(
					add_query_arg(
						array(
			...
			'placeholder'      => __( 'The number of subscriptions purchased in parent orders created during this period. This represents the new subscriptions created by customers placing an order via checkout.', 'skyhs-hosting-solution' ),
		...
		$legend[] = array(
			'title'            => sprintf(
				// translators: 2: link opening tag, 1: subscription count and closing tag.
				__( '%2$s %1$s subscription resubscribes', 'skyhs-hosting-solution' ),
				'<strong> <span class="skyhs-hosting-solution-count count">' . $this->report_data->resubscribe_orders_total_count . '</strong> </a>',
				'<a href="' .
				esc_url(
					add_query_arg(
						array(
			...
			'placeholder'      => __( 'The number of resubscribe orders processed during this period.', 'skyhs-hosting-solution' ),
		...
		$legend[] = array(
			'title'            => sprintf(
				// translators: 2: link opening tag, 1: subscription count and closing tag.
				__( '%2$s %1$s subscription renewals', 'skyhs-hosting-solution' ),
				'<strong> <span class="skyhs-hosting-solution-count count">' . $this->report_data->renewal_orders_total_count . '</strong> </a>',
				'<a href="' .
				esc_url(
					add_query_arg(
						array(
			...
			'placeholder'      => __( 'The number of renewal orders processed during this period.', 'skyhs-hosting-solution' ),
		...
		$legend[] = array(
			'title'            => sprintf(
				// translators: 2: link opening tag, 1: subscription count and closing tag.
				__( '%2$s %1$s subscription switches', 'skyhs-hosting-solution' ),
				'<strong> <span class="skyhs-hosting-solution-count count">' . $this->report_data->switch_orders_total_count . '</strong> </a>',
				'<a href="' .
				esc_url(
					add_query_arg(
						array(
			...
			'placeholder'      => __( 'The number of subscriptions upgraded, downgraded or cross-graded during this period.', 'skyhs-hosting-solution' ),
		...
		$legend[] = array(
			'title'            => sprintf(
				// translators: 2: link opening tag, 1: subscription count and closing tag.
				__( '%2$s %1$s subscription cancellations', 'skyhs-hosting-solution' ),
				'<strong> <span class="skyhs-hosting-solution-count count">' . $this->report_data->total_subscriptions_cancelled . '</strong> </a>',
				'<a href="' .
				esc_url(
					add_query_arg(
						array(
			...
			'placeholder'      => __( 'The number of subscriptions cancelled by the customer or store manager during this period.  The pre-paid term may not yet have ended during this period.', 'skyhs-hosting-solution' ),
		...
		$legend[] = array(
			'title'            => sprintf(
				// translators: 2: link opening tag, 1: subscription count and closing tag.
				__( '%2$s %1$s ended subscriptions', 'skyhs-hosting-solution' ),
				'<strong> <span class="skyhs-hosting-solution-count count">' . $this->report_data->total_subscriptions_ended . '</strong> </a>',
				'<a href="' .
				esc_url(
					add_query_arg(
						array(
			...
			'placeholder'      => __( 'The number of subscriptions which have either expired or reached the end of the prepaid term if it was previously cancelled.', 'skyhs-hosting-solution' ),
		...
		$legend[] = array(
			'title'            => sprintf(
				// translators: 2: link opening tag, 1: subscription count and closing tag.
				__( '%2$s %1$s current subscriptions', 'skyhs-hosting-solution' ),
				'<strong> <span class="skyhs-hosting-solution-count count">' . $this->report_data->total_subscriptions_at_period_end . '</strong> </a>',
				'<a href="' .
				esc_url(
					add_query_arg(
						array(
			...
			'placeholder'      => __( 'The number of subscriptions during this period with an end date in the future and a status other than pending.', 'skyhs-hosting-solution' ),
		...
		if ( $data->total_subscriptions_at_period_end - $data->total_subscriptions_at_period_start >= 0 ) {
			...
			$legend_title = __( '%s net subscription gain', 'skyhs-hosting-solution' );
		} else {
			// translators: %s: subscription net loss (with percentage).
			$legend_title = __( '%s net subscription loss', 'skyhs-hosting-solution' );
		}
		...
		$legend[] = array(
			...
			'placeholder'      => __( 'Change in subscriptions between the start and end of the period.', 'skyhs-hosting-solution' ),
	...
	public function output_report() {
		...
		$ranges = array(
			...
			'last_month' => __( 'Last Month', 'skyhs-hosting-solution' ),
			'month'      => __( 'This Month', 'skyhs-hosting-solution' ),
			'7day'       => __( 'Last 7 Days', 'skyhs-hosting-solution' ),
		);
	...
	public function get_export_button() {
		...
		<a
			...
			class="export_csv"
			data-export="chart"
			data-xaxes="<?php esc_attr_e( 'Date', 'skyhs-hosting-solution' ); ?>"
			data-exclude_series="2"
			data-groupby="<?php echo esc_attr( $this->chart_groupby ); ?>"
		>
			<?php esc_attr_e( 'Export CSV', 'skyhs-hosting-solution' ); ?>
		</a>
		<?php
	}
		...
		<script type="text/javascript">
			...
			jQuery(function(){
				...
				var drawGraph = function( highlight ) {
					var series = [
						{
							label: "<?php echo esc_js( __( 'Switched subscriptions', 'skyhs-hosting-solution' ) ) ?>",
						...
						{
							label: "<?php echo esc_js( __( 'New Subscriptions', 'skyhs-hosting-solution' ) ) ?>",
						...
						{
							label: "<?php echo esc_js( __( 'Subscriptions signups', 'skyhs-hosting-solution' ) ) ?>",
						...
						{
							label: "<?php echo esc_js( __( 'Number of resubscribes', 'skyhs-hosting-solution' ) ) ?>",
						...
						{
							label: "<?php echo esc_js( __( 'Number of renewals', 'skyhs-hosting-solution' ) ) ?>",
						...
						{
							label: "<?php echo esc_js( __( 'Subscriptions', 'skyhs-hosting-solution' ) ) ?>",
						...
						{
							label: "<?php echo esc_js( __( 'Subscriptions Ended', 'skyhs-hosting-solution' ) ) ?>",
						...
						{
							label: "<?php echo esc_js( __( 'Cancellations', 'skyhs-hosting-solution' ) ) ?>",
						...
						{
							label: "<?php echo esc_js( __( 'Signup Totals', 'skyhs-hosting-solution' ) ) ?>",
						...
						{
							label: "<?php echo esc_js( __( 'Resubscribe Totals', 'skyhs-hosting-solution' ) ) ?>",
						...
						{
							label: "<?php echo esc_js( __( 'Renewal Totals', 'skyhs-hosting-solution' ) ) ?>",
						...
						{
							label: "<?php echo esc_js( __( 'Switch Totals', 'skyhs-hosting-solution' ) ) ?>",
							...
							yaxes: [
								...
								},
								{
									...
									tickFormatter: function (tick) {
										...
										return jQuery.skyhshoso_format_money(tick,0);
									},
