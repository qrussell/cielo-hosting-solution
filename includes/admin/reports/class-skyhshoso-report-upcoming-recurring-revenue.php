 ...
 * @since      2.1
 */
class SkyHSHOSO_Report_Upcoming_Recurring_Revenue extends WC_Admin_Report {
	...
	public function get_chart_legend() {
		...
		foreach ( $this->order_ids_recurring_totals as $r ) {
			...
			foreach ( $subscription_ids as $key => $subscription_id ) {
				...
				do {
					...
					$next_payment_timestamp = skyhshoso_add_time( $billing_intervals[ $key ], $billing_periods[ $key ], $next_payment_timestamp );
		...
		$legend[] = array(
			...
			'title'            => sprintf( __( '%s renewal income in this period', 'skyhs-hosting-solution' ), '<strong>' . wc_price( $total_renewal_revenue ) . '</strong>' ),
			'placeholder'      => __( 'The sum of all the upcoming renewal orders, including items, fees, tax and shipping, for currently active subscriptions.', 'skyhs-hosting-solution' ),
			'color'            => $this->chart_colours['renewals_amount'],
			'highlight_series' => 1,
		);
		$legend[] = array(
			// translators: %s: renewal count.
			'title'            => sprintf( __( '%s renewal orders', 'skyhs-hosting-solution' ), '<strong>' . $total_renewal_count . '</strong>' ),
			'placeholder'      => __( 'The number of upcoming renewal orders, for currently active subscriptions.', 'skyhs-hosting-solution' ),
			'color'            => $this->chart_colours['renewals_count'],
			'highlight_series' => 0,
		);
		$legend[] = array(
			// translators: %s: formatted amount.
			'title' => sprintf( __( '%s average renewal amount', 'skyhs-hosting-solution' ), '<strong>' . wc_price( $this->average_sales ) . '</strong>' ),
	...
	public function get_data( $args = array() ) {
		...
		$args = apply_filters( 'skyhshoso_reports_upcoming_recurring_revenue_args', $args );
		...
		if ( $args['no_cache'] || ! isset( $cached_results[ $query_hash ] ) ) {
			...
			$cached_results[ $query_hash ] = apply_filters( 'skyhshoso_reports_upcoming_recurring_revenue_data', $wpdb->get_results( $base_query, OBJECT_K ), $args );
	...
	public function output_report() {
		...
		$ranges = array(
			'year'       => __( 'Next 12 Months', 'skyhs-hosting-solution' ),
			'month'      => __( 'Next 30 Days', 'skyhs-hosting-solution' ),
			'last_month' => __( 'Next Month', 'skyhs-hosting-solution' ), // misnomer to match historical reports keys, handy for caching
			'7day'       => __( 'Next 7 Days', 'skyhs-hosting-solution' ),
		);
	...
	public function get_export_button() {
		...
		>
			<?php esc_html_e( 'Export CSV', 'skyhs-hosting-solution' ); ?>
		</a>
		<?php
	}
	...
	public function get_main_chart() {
		...
		?>
		<div id="skyhs_hosting_solution_upcoming_recurring_revenue_chart" class="chart-container">
		...
		<script type="text/javascript">
			...
			jQuery(function(){
				...
				var drawGraph = function( highlight ) {
					var series = [
						{
							label: "<?php echo esc_js( __( 'Renewals count', 'skyhs-hosting-solution' ) ) ?>",
						...
						{
							label: "<?php echo esc_js( __( 'Renewals amount', 'skyhs-hosting-solution' ) ) ?>",
	...
	public function calculate_current_range( $current_range ) {
		switch ( $current_range ) {
			case 'custom':
				$this->start_date = isset( $_GET['start_date'] ) ? strtotime( sanitize_text_field( wp_unslash( $_GET['start_date'] ) ) ) : 0; // phpcs:ignore WordPress.Security.NonceVerification.Recommended
				$this->end_date   = isset( $_GET['end_date'] ) ? strtotime( 'midnight', strtotime( sanitize_text_field( wp_unslash( $_GET['end_date'] ) ) ) ) : 0; // phpcs:ignore WordPress.Security.NonceVerification.Recommended
				...
				while ( ( $min_date = skyhshoso_add_months( $min_date, '1' ) ) <= $this->end_date ) {
					$interval ++;
				}
			...
			case 'last_month': // misnomer to match historical reports keys, handy for caching
				$this->start_date     = strtotime( date( 'Y-m-01', skyhshoso_add_months( current_time( 'timestamp' ), '1' ) ) );
				$this->end_date       = strtotime( date( 'Y-m-t', $this->start_date ) );
				$this->chart_groupby  = 'day';
			break;
			case 'month':
				$this->start_date    = strtotime( 'now', current_time( 'timestamp' ) );
				$this->end_date      = skyhshoso_add_months( current_time( 'timestamp' ), '1' );
				$this->chart_groupby = 'day';
			break;
			case '7day':
				$this->start_date    = strtotime( 'now', current_time( 'timestamp' ) );
