 ...
 * @subpackage SkyHSHOSO_Cart_Renewal
 * @category   Class
 * @since      1.0.0
 */

class SkyHSHOSO_Cart_Renewal {
	...
	public function __construct() {
		...
		add_action( 'skyhshoso_before_renewal_setup_cart_subscriptions', array( &$this, 'clear_coupons' ), 10 );

		// Handles renew of password-protected products.
		add_action( 'skyhshoso_before_renewal_setup_cart_subscriptions', 'skyhshoso_allow_protected_products_to_renew' );
		add_action( 'skyhshoso_after_renewal_setup_cart_subscriptions', 'skyhshoso_disallow_protected_product_add_to_cart_validation' );
	...
	public function attach_dependant_hooks() {
		...
		if ( skyhshoso_is_woocommerce_pre( '3.0' ) ) {
	...
	public function setup_hooks() {
		...
		skyhshoso_add_woocommerce_dependent_action( 'woocommerce_before_cart_item_quantity_zero', array( &$this, 'maybe_remove_items' ), '3.7.0', '<' );
	...
	public function maybe_setup_cart() {
		...
		if ( isset( $_GET['pay_for_order'] ) && isset( $_GET['key'] ) && isset( $wp->query_vars['order-pay'] ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Recommended

			// Pay for existing order
			$order_key = sanitize_text_field( wp_unslash( $_GET['key'] ) ); // phpcs:ignore WordPress.Security.NonceVerification.Recommended
			$order_id  = isset( $wp->query_vars['order-pay'] ) ? $wp->query_vars['order-pay'] : ( isset( $_GET['order_id'] ) ? absint( wp_unslash( $_GET['order_id'] ) ) : 0 ); // phpcs:ignore WordPress.Security.NonceVerification.Recommended
			$order     = wc_get_order( $order_id );

			if ( skyhshoso_get_objects_property( $order, 'order_key' ) === $order_key && $order->has_status( array( 'pending', 'failed' ) ) && skyhshoso_order_contains_renewal( $order ) ) {

				// If a user isn't logged in, allow them to login first and then redirect back
				if ( ! is_user_logged_in() ) {
					$redirect = add_query_arg(
						array(
							'skyhshoso_redirect'    => 'pay_for_order',
							'skyhshoso_redirect_id' => $order_id,
						),
					...
					wc_add_notice( __( 'That doesn\'t appear to be your order.', 'skyhs-hosting-solution' ), 'error' );
					wp_safe_redirect( get_permalink( wc_get_page_id( 'myaccount' ) ) );
					exit;
				}

				$subscriptions = skyhshoso_get_subscriptions_for_renewal_order( $order );

				do_action( 'skyhshoso_before_renewal_setup_cart_subscriptions', $subscriptions, $order );

				foreach ( $subscriptions as $subscription ) {

					do_action( 'skyhshoso_before_renewal_setup_cart_subscription', $subscription, $order );

					// Check if order/subscription can be paid for
					if ( empty( $subscription ) || ! $subscription->has_status( array( 'on-hold', 'pending' ) ) ) {
						wc_add_notice( __( 'This order can no longer be paid because the corresponding subscription does not require payment at this time.', 'skyhs-hosting-solution' ), 'error' );
					} else {
					...
					do_action( 'skyhshoso_after_renewal_setup_cart_subscription', $subscription, $order );
				}

				do_action( 'skyhshoso_after_renewal_setup_cart_subscriptions', $subscriptions, $order );

				if ( WC()->cart->cart_contents_count != 0 ) {
					// Store renewal order's ID in session so it can be re-used after payment
					$this->set_order_awaiting_payment( $order_id );
					wc_add_notice( __( 'Complete checkout to renew your subscription.', 'skyhs-hosting-solution' ), 'success' );
				}
	...
	protected function setup_cart( $subscription, $cart_item_data, $validation_type = 'all_items_not_required' ) {
		...
		foreach ( $subscription->get_items() as $item_id => $line_item ) {
			...
			$product_deleted_error_message = apply_filters( 'skyhs_hosting_solution_renew_deleted_product_error_message', __( 'The %s product has been deleted and can no longer be renewed. Please choose a new product or contact us for assistance.', 'skyhs-hosting-solution' ) );
		...
		if ( ! $success && 'all_items_required' === $validation_type ) {
			if ( skyhshoso_is_subscription( $subscription ) ) {
				// translators: %s is subscription's number
				wc_add_notice( sprintf( esc_html__( 'Subscription #%s has not been added to the cart.', 'skyhs-hosting-solution' ), $subscription->get_order_number() ), 'error' );
			} else {
				// translators: %s is order's number
				wc_add_notice( sprintf( esc_html__( 'Order #%s has not been added to the cart.', 'skyhs-hosting-solution' ), $subscription->get_order_number() ), 'error' );
			}
	...
	public function cart_items_loaded_from_session( $cart ) {
		...
		foreach ( $cart->cart_contents as $key => $item ) {
			if ( isset( $item[ $this->cart_item_key ]['subscription_id'] ) && ! skyhshoso_is_subscription( $item[ $this->cart_item_key ]['subscription_id'] ) ) {
		...
		if ( $removed_count_subscription ) {
			$error_message = esc_html( _n( 'We couldn\'t find the original subscription for an item in your cart. The item was removed.', 'We couldn\'t find the original subscriptions for items in your cart. The items were removed.', $removed_count_subscription, 'skyhs-hosting-solution' ) );
		...
		if ( $removed_count_order ) {
			$error_message = esc_html( _n( 'We couldn\'t find the original renewal order for an item in your cart. The item was removed.', 'We couldn\'t find the original renewal orders for items in your cart. The items were removed.', $removed_count_order, 'skyhs-hosting-solution' ) );
			if ( ! wc_has_notice( $error_message, 'notice' ) ) {
				wc_add_notice( $error_message, 'notice' );
			}
		}
			...
			if ( $subscription ) {
				...
				if ( $_product->is_taxable() && $subscription->get_prices_include_tax() ) {
					...
					if ( isset( $item_to_renew['_subtracted_base_location_tax'] ) ) {
						SkyHSHOSO_Upgrader::repair_subtracted_base_taxes( $item_to_renew->get_id() );
				...
				skyhshoso_set_objects_property( $_product, 'subscription_sign_up_fee', 0, 'set_prop_only' );
				...
				skyhshoso_set_objects_property( $_product, 'name', apply_filters( 'skyhs_hosting_solution_renewal_product_title', $line_item_name, $_product ), 'set_prop_only' );
	 ...
	 *
	 * @deprecated subscriptions-core 5.3.0 - This method has moved to the SkyHSHOSO_Checkout class.
	 *
	...
	public function maybe_check_ship_to_different_address( $ship_to_different_address ) {
		skyhshoso_deprecated_function( __METHOD__, '5.3.0', 'SkyHSHOSO_Checkout::maybe_check_ship_to_different_address( $ship_to_different_address )' );
	...
	public function maybe_update_subscription_customer_data( $update_customer_data, $checkout_object ) {
		...
		if ( false !== $cart_renewal_item ) {
			...
			$subscription = skyhshoso_get_subscription( $cart_renewal_item[ $this->cart_item_key ]['subscription_id'] );
	...
	public function is_purchasable( $is_purchasable, $product ) {
		_deprecated_function( __METHOD__, '2.1', 'SkyHSHOSO_Limiter::is_purchasable_renewal' );
		return SkyHSHOSO_Limiter::is_purchasable_renewal( $is_purchasable, $product );
	...
	public function get_checkout_payment_url( $pay_url, $order ) {
		...
		if ( skyhshoso_order_contains_renewal( $order ) ) {
	...
	public function maybe_remove_items( $cart_item_key ) {
		...
		if ( isset( WC()->cart->cart_contents[ $cart_item_key ][ $this->cart_item_key ]['subscription_id'] ) ) {
			...
			if ( $removed_item_count > 1 && 'woocommerce_before_cart_item_quantity_zero' == current_filter() ) {
				wc_add_notice( esc_html__( 'All linked subscription items have been removed from the cart.', 'skyhs-hosting-solution' ), 'notice' );
			}
		}
	}
	...
	public function items_removed_title( $product_title, $cart_item ) {
		...
		if ( isset( $cart_item[ $this->cart_item_key ]['subscription_id'] ) ) {
			$subscription  = $this->get_order( $cart_item );
			$product_title = ( count( $subscription->get_items() ) > 1 ) ? esc_html_x( 'All linked subscription items were', 'Used in WooCommerce by removed item notification: "_All linked subscription items were_ removed. Undo?" Filter for item title.', 'skyhs-hosting-solution' ) : $product_title;
		}
	...
	public function renewal_coupon_data( $data, $code ) {
		...
		$renewal_coupons = WC()->session->get( 'skyhshoso_renewal_coupons' );
		...
		foreach ( $renewal_coupons as $order_id => $coupons ) {
			...
			foreach ( $coupons as $coupon_code => $coupon_properties ) {
				...
				if ( $coupon_code == $code ) {
					$expiry_date_property = skyhshoso_is_woocommerce_pre( '3.0' ) ? 'expiry_date' : 'date_expires';
	 ...
	 *
	 * @param  object WC_Order | SkyHSHOSO_Subscription $order
	...
	protected function store_coupon( $order_id, $coupon ) {
		if ( ! empty( $order_id ) && ! empty( $coupon ) ) {
			$renewal_coupons   = WC()->session->get( 'skyhshoso_renewal_coupons', array() );
			$use_bools         = skyhshoso_is_woocommerce_pre( '3.0' ); // Some coupon properties have changed from accepting 'no' and 'yes' to true and false args.
			...
			if ( array_key_exists( $order_id, $renewal_coupons ) ) {
				$renewal_coupons[ $order_id ][ skyhshoso_get_coupon_property( $coupon, 'code' ) ] = $coupon_properties;
			} else {
				$renewal_coupons[ $order_id ] = array( skyhshoso_get_coupon_property( $coupon, 'code' ) => $coupon_properties );
			}

			WC()->session->set( 'skyhshoso_renewal_coupons', $renewal_coupons );
		}
	}
	...
	public function clear_coupons() {
		...
		WC()->session->set( 'skyhshoso_renewal_coupons', array() );
	}
	...
	public function maybe_redirect_after_login( $redirect, $user = null ) {
		 ...
		 */
		if ( isset( $_GET['skyhshoso_redirect'], $_GET['skyhshoso_redirect_id'] ) && 'pay_for_order' === $_GET['skyhshoso_redirect'] ) { // phpcs:ignore WordPress.Security.NonceVerification.Recommended
			$order = wc_get_order( sanitize_text_field( wp_unslash( $_GET['skyhshoso_redirect_id'] ) ) ); // phpcs:ignore WordPress.Security.NonceVerification.Recommended

			if ( $order && $order->get_user_id() && user_can( $user, 'pay_for_order', $order->get_id() ) ) {
				$redirect = $order->get_checkout_payment_url();
			} else {
				// Remove the skyhshoso_redirect query args if the user doesn't have permission to pay for the order.
				$redirect = remove_query_arg( array( 'skyhshoso_redirect', 'skyhshoso_redirect_id' ), $redirect );
			}
		}
	 ...
	 *
	 * This is required so that changes made by @see SkyHSHOSO_Cart_Renewal->add_line_item_meta() (or @see
	 * SkyHSHOSO_Cart_Renewal->update_line_item_cart_data() for WC < 3.0), are also reflected
	 * in the session cart.
	 *
	 * @since 1.0.0
	 */
	...
	*/
	 ...
	 * @see SkyHSHOSO_Cart_Renewal->update_line_item_cart_data()
	 *
	...
	public function hidden_order_itemmeta( $hidden_meta_keys ) {
		...
		if ( apply_filters( 'skyhs_hosting_solution_hide_itemmeta', ! defined( 'SkyHSHOSO_DEBUG' ) || true !== SkyHSHOSO_DEBUG ) ) {
	...
	public function maybe_update_subscription_address_data_from_store_api( $customer, $request ) {
		...
		if ( false !== $cart_renewal_item ) {
			...
			if ( skyhshoso_is_woocommerce_pre( '7.1' ) ) {
	 ...
	 * @see SkyHSHOSO_Cart_Renewal->add_order_line_item_meta() replaces this function
	 *
	 * @param int $item_id
	 * @param array $cart_item_data
	 * @since 1.0.0
	 ...
	 *
	 * Used when WC 3.0 or newer is active. When prior versions are active,
	 * @see SkyHSHOSO_Cart_Renewal->add_order_item_meta() replaces this function
	 *
	...
	public function remove_non_recurring_fees( $cart ) {
		...
		foreach ( $cart_fees as $fee ) {
			...
			if ( true === apply_filters( 'skyhs_hosting_solution_is_recurring_fee', $is_recurring_fee, $fee, $cart ) ) {
				if ( is_callable( array( $cart, 'fees_api' ) ) ) { // WC 3.2 +
	...
	public function maybe_update_shipping_packages( $packages ) {
		...
		foreach ( $packages as $index => $package ) {
			foreach ( $package['contents'] as $cart_item_key => $cart_item ) {
				if ( SkyHSHOSO_Product::needs_one_time_shipping( $cart_item['data'] ) ) {
	...
	public function maybe_preserve_order_created_via( $order ) {
		...
		if ( isset( $changes['created_via'], $current_data['created_via'] ) && 'subscription' === $current_data['created_via'] && 'checkout' === $changes['created_via'] && skyhshoso_order_contains_renewal( $order ) ) {
	...
	public function maybe_disable_manual_renewal_stock_validation() {
		if ( apply_filters( 'skyhs_hosting_solution_disable_manual_renewal_stock_validation', false ) ) {
			SkyHSHOSO_Renewal_Cart_Stock_Manager::attach_callbacks();
		}
	}
	...
	public function order_button_text( $place_order_text ) {
		if ( isset( WC()->cart ) && count( skyhshoso_get_order_type_cart_items( 'renewal' ) ) === count( WC()->cart->get_cart() ) ) {
			$place_order_text = _x( 'Renew subscription', 'The place order button text while renewing a subscription', 'skyhs-hosting-solution' );
		}
	 ...
	 * This function behaves similarly to @see SkyHSHOSO_Cart_Renewal::update_cart_hash() for the standard checkout and is hooked onto the 'woocommerce_create_order' filter.
	 *
	...
	public function update_line_item_cart_data( $item_id, $cart_item_data, $cart_item_key ) {
		...
		if ( false === skyhshoso_is_woocommerce_pre( '3.0' ) ) {
	 ...
	 *
	 * @param SkyHSHOSO_Subscription $subscription subscription
	 * @param WC_Order $order
	 *
	 * @since 1.0.0
	 * @deprecated 1.0.0
	...
	public function maybe_setup_discounts( $subscription, $order = null ) {
		skyhshoso_deprecated_function( __METHOD__, '2.4.3' );
		...
		if ( skyhshoso_is_subscription( $order ) || skyhshoso_order_contains_renewal( $order ) ) {

			$used_coupons   = skyhshoso_get_used_coupon_codes( $order );
			$order_discount = skyhshoso_get_objects_property( $order, 'cart_discount' );
			...
			if ( ! empty( $used_coupons ) ) {
				...
				foreach ( $coupon_items as $coupon_item ) {
					...
					$coupon      = new WC_Coupon( $coupon_item->get_name() );
					$coupon_type = skyhshoso_get_coupon_property( $coupon, 'discount_type' );
					$coupon_code = '';

					// If the coupon still exists we can use the existing/available coupon properties
					if ( true === skyhshoso_get_coupon_property( $coupon, 'exists' ) ) {
						...
						if ( in_array( $coupon_type, array( 'recurring_percent', 'recurring_fee' ) ) ) {
							...
							if ( 'recurring_percent' == $coupon_type ) {
								skyhshoso_set_coupon_property( $coupon, 'discount_type', 'renewal_percent' );
							} elseif ( 'recurring_fee' == $coupon_type ) {
								skyhshoso_set_coupon_property( $coupon, 'discount_type', 'renewal_fee' );
							}

							// Adjust coupon code to reflect that it is being applied to a renewal
							$coupon_code = skyhshoso_get_coupon_property( $coupon, 'code' );
						}
					} else {
						...
						skyhshoso_set_coupon_property( $coupon, 'discount_type', 'renewal_cart' );
						...
						skyhshoso_set_coupon_property( $coupon, 'coupon_amount', $coupon_amount );
					}
					...
					if ( ! empty( $coupon_code ) ) {
						...
						skyhshoso_set_coupon_property( $coupon, 'product_ids', $this->get_products( $order ) );

						// Store the coupon info for later
						$this->store_coupon( skyhshoso_get_objects_property( $order, 'id' ), $coupon );
				...
				skyhshoso_set_coupon_property( $coupon, 'coupon_amount', $order_discount );
	...
	public function maybe_preserve_order_status( $order_status ) {
		skyhshoso_deprecated_function( __METHOD__, '6.3.0' );
		if ( null !== WC()->session && 'failed' !== $order_status ) {
			...
			if ( $order && skyhshoso_order_contains_renewal( $order ) && $order->has_status( 'failed' ) ) {
				$order_status = 'failed';
			}
