...
class SkyHSHOSO_Limited_Recurring_Coupon_Manager {
	 ...
	 * @var string
	 */
	private static $coupons_renewals = '_skyhshoso_number_payments';
	...
	public static function init() {
		...
		add_filter( 'skyhshoso_bypass_coupon_removal', array( __CLASS__, 'maybe_remove_coupons_from_recurring_cart' ), 1000, 5 );
	}
	...
	public static function add_coupon_fields( $id ) {
		...
		woocommerce_wp_text_input( array(
			'id'          => 'skyhshoso_number_payments',
			'label'       => __( 'Active for x payments', 'skyhs-hosting-solution' ),
			'placeholder' => __( 'Unlimited payments', 'skyhs-hosting-solution' ),
			'description' => __( 'Coupon will be limited to the given number of payments. It will then be automatically removed from the subscription. "Payments" also includes the initial subscription payment.', 'skyhs-hosting-solution' ),
			'desc_tip'    => true,
			'data_type'   => 'decimal',
	...
	public static function save_coupon_fields( $id ) {
		...
		$coupon = new WC_Coupon( $id );
		$number_payments = isset( $_POST['skyhshoso_number_payments'] ) ? sanitize_text_field( wp_unslash( $_POST['skyhshoso_number_payments'] ) ) : '';
		$coupon->add_meta_data( self::$coupons_renewals, wc_clean( $number_payments ), true );
		$coupon->save();
	}
	...
	public static function get_coupon_limit( $code ) {
		if ( skyhshoso_is_woocommerce_pre( '3.2' ) ) {
			return false;
		}
		...
		if ( SkyHSHOSO_Coupon::is_renewal_cart_coupon( $coupon_type ) ) {
			$coupon      = SkyHSHOSO_Coupon::map_virtual_coupon( $code );
			$coupon_type = $coupon->get_discount_type();
		}

		$limited = $coupon->get_meta( self::$coupons_renewals );

		return SkyHSHOSO_Coupon::is_recurring_coupon( $coupon_type ) ? intval( $limited ) : false;
	}
	...
	public static function order_has_limited_recurring_coupon( $order ) {
		...
		foreach ( skyhshoso_get_used_coupon_codes( $order ) as $code ) {
			if ( self::coupon_is_limited( $code ) ) {
				$has_coupon = true;
				break;
			}
	 ...
	 *
	 * @since 4.0.0
	 * @param SkyHSHOSO_Subscription $subscription The current subscription.
	 */
	public static function check_coupon_usages( $subscription ) {
		// If there aren't any coupons, there's nothing to do.
		$coupons = skyhshoso_get_used_coupon_codes( $subscription );
		if ( empty( $coupons ) ) {
			return;
		}
	...
	public static function add_limit_to_list_table( $column_name, $id ) {
		...
		if ( $limit ) {
			echo esc_html( sprintf(
				...
				_n( 'Active for %d payment', 'Active for %d payments', $limit, 'skyhs-hosting-solution' ),
				number_format_i18n( $limit )
			) );
		} else {
			esc_html_e( 'Active for unlimited payments', 'skyhs-hosting-solution' );
		}
	}
	...
	public static function recurring_cart_contains_expiring_coupon( $recurring_cart ) {
		...
		$subscription_length   = skyhshoso_cart_pluck( $recurring_cart, 'subscription_length' );
		$subscription_payments = $subscription_length / skyhshoso_cart_pluck( $recurring_cart, 'subscription_period_interval' );
		...
		if ( empty( $subscription_length ) ) {
			...
			foreach ( $limited_recurring_coupons as $code ) {
				if ( SkyHSHOSO_Coupon::get_coupon_limit( $code ) < $subscription_payments ) {
					$has_expiring_coupon = true;
					break;
				}
			}
	...
	public static function gateways_subscription_amount_changes( $gateways ) {
		...
		$change_payment     = isset( $_GET['change_payment_method'] ) ? sanitize_text_field( wp_unslash( $_GET['change_payment_method'] ) ) : 0; // phpcs:ignore WordPress.Security.NonceVerification.Recommended
		...
		if ( $change_payment && isset( $_GET['_wpnonce'] ) && wp_verify_nonce( sanitize_text_field( wp_unslash( $_GET['_wpnonce'] ) ) ) ) {
			$subscription       = skyhshoso_get_subscription( $change_payment );
	...
	public static function no_available_payment_methods_message() {
		return __( 'Sorry, it seems there are no available payment methods which support the recurring coupon you are using. Please contact us if you require assistance or wish to make alternate arrangements.', 'skyhs-hosting-solution' );
	}
	 ...
	 * @param string    $calculation_type      The WC Subscriptions cart calculation mode. Can be 'recurring_total' or 'none'. @see SkyHSHOSO_Cart::get_calculation_type()
	 *
	...
	public static function maybe_remove_coupons_from_recurring_cart( $bypass_default_checks, $coupon, $coupon_type, $calculation_type, $cart ) {
		...
		if ( ! SkyHSHOSO_Coupon::is_recurring_coupon( $coupon_type ) ) {
			return $bypass_default_checks;
		}
