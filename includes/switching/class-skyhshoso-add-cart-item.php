<?php
/**
 * SkyHS Hosting Solution Add Cart Item.
 *
 * A class to assist in the calculations required to add an item to an existing subscription.
 * To enable proration, adding a product to a subscription inherits all the switch item (@see SkyHSHOSO_Switch_Cart_Item) functionality, however, doesn't have an existing item (@see SkyHSHOSO_Switch_Cart_Item::$existing_item) to replace.
 *
 * @package SkyHS Hosting Solution
 * @since   2.6.0
 */
class SkyHSHOSO_Add_Cart_Item extends SkyHSHOSO_Switch_Cart_Item {
	...
	public function get_old_price_per_day() {
		if ( ! isset( $this->old_price_per_day ) ) {
			$this->old_price_per_day = apply_filters( 'skyhshoso_switch_proration_old_price_per_day', 0, $this->subscription, $this->cart_item, 0, $this->get_days_in_old_cycle() );
		}
