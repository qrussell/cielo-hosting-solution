<?php
/**
 * SkyHS Hosting Solution Switch Totals Calculator.
 *
 ...
 * @since   2.6.0
 */
class SkyHSHOSO_Switch_Totals_Calculator {
	 ...
	 *
	 * @var SkyHSHOSO_Switch_Cart_Item[]
	 */
	...
	protected function load_settings() {
		$this->apportion_recurring_price = get_option( 'skyhshoso' . '_apportion_recurring_price', 'no' );
		$this->apportion_sign_up_fee     = get_option( 'skyhshoso' . '_apportion_sign_up_fee', 'no' );
		$this->apportion_length          = get_option( 'skyhshoso' . '_apportion_length', 'no' );
	...
	public function calculate_prorated_totals() {
		foreach ( $this->get_switches_from_cart() as $cart_item_key => $switch_item ) {
			...
			if ( $this->should_prorate_recurring_price( $switch_item ) ) {
				...
				if ( apply_filters( 'skyhshoso_switch_should_extend_prepaid_term', 'downgrade' === $switch_type && $this->should_extend_prepaid_term(), $switch_item ) ) {
			...
			do_action( 'skyhshoso_switch_calculations_completed', $switch_item, $cart_item_key );
	...
	/**
	 * Gets all the switch items in the cart as instances of @see SkyHSHOSO_Switch_Cart_Item.
	 *
	 * @since 2.6.0
	 * @return SkyHSHOSO_Switch_Cart_Item[]
	 */
	protected function get_switches_from_cart() {
		...
		foreach ( $this->cart->get_cart() as $cart_item_key => $cart_item ) {
			...
			if ( ! empty( $cart_item['subscription_switch']['item_id'] ) ) {
				...
				$existing_item = skyhshoso_get_order_item( $cart_item['subscription_switch']['item_id'], $subscription );
				...
				$switch_item = new SkyHSHOSO_Switch_Cart_Item( $cart_item, $subscription, $existing_item );
			} else {
				$switch_item = new SkyHSHOSO_Add_Cart_Item( $cart_item, $subscription );
			}
			 ...
			 * @since 3.0.0
			 *
			 * @param SkyHSHOSO_Switch_Cart_Item $switch_item The switch item.
			 * @param array $cart_item The item in the cart the switch item was created for.
			 * @param string $cart_item_key The cart item key.
			 */
			$switches[ $cart_item_key ] = apply_filters( 'skyhshoso_proration_switch_item_from_cart_item', $switch_item, $cart_item, $cart_item_key );

			// Ensure the filtered item is the correct object type.
			if ( ! is_a( $switches[ $cart_item_key ], 'SkyHSHOSO_Switch_Cart_Item' ) ) {
				unset( $switches[ $cart_item_key ] );
				WC()->cart->remove_cart_item( $cart_item_key );

				$error_notice = __( 'Your cart contained an invalid subscription switch request. It has been removed from your cart.', 'skyhs-hosting-solution' );
	 ...
	 *
	 * @since 2.6.0
	 * @param SkyHSHOSO_Switch_Cart_Item $switch_item
	 * @return bool
	 */
	protected function should_prorate_recurring_price( $switch_item ) {
		...
		return apply_filters( 'skyhshoso_switch_should_prorate_recurring_price', $prorate_all || ( $prorate_virtual && $switch_item->is_virtual_product() ), $switch_item );
	}
	...
	protected function should_reduce_prepaid_term( $switch_item ) {
		 ...
		 */
		return apply_filters( 'skyhshoso_switch_proration_reduce_pre_paid_term', $is_switch_out_of_trial || $days_in_old_cycle > $days_in_new_cycle, $switch_item->subscription, $switch_item->cart_item, $days_in_old_cycle, $days_in_new_cycle, $switch_item->get_old_price_per_day(), $switch_item->get_new_price_per_day() );
	}
	...
	protected function should_apportion_length( $switch_item ) {
		...
		return apply_filters( 'skyhshoso_switch_should_prorate_length', 'yes' == $this->apportion_length || ( 'virtual' == $this->apportion_length && $switch_item->is_virtual_product() ), $switch_item );
	}
	...
	protected function apportion_sign_up_fees( $switch_item ) {
		$should_apportion_sign_up_fee = apply_filters( 'skyhshoso_switch_should_prorate_sign_up_fee', 'yes' === $this->apportion_sign_up_fee, $switch_item );
		...
		if ( $should_apportion_sign_up_fee && $switch_item->existing_item ) {
			...
			$sign_up_fee_due  = SkyHSHOSO_Subscriptions_Product::get_sign_up_fee( $product );
			...
			$subscription_sign_up_fee = apply_filters( 'skyhshoso_switch_sign_up_fee', max( $sign_up_fee_due - $sign_up_fee_paid, 0 ), $switch_item );

			$switch_item->product->update_meta_data( '_subscription_sign_up_fee',  $subscription_sign_up_fee );
			$switch_item->product->update_meta_data( '_subscription_sign_up_fee_prorated', SkyHSHOSO_Subscriptions_Product::get_sign_up_fee( $switch_item->product ) );
		} elseif ( 'no' === $this->apportion_sign_up_fee ) {
			// Allowing third parties to force the application of a sign-up fee
			$subscription_sign_up_fee = apply_filters( 'skyhshoso_switch_sign_up_fee', 0, $switch_item );
	...
	protected function calculate_upgrade_cost( $switch_item ) {
		...
		if ( ! $switch_item->is_switch_during_trial() && 1 == SkyHSHOSO_Subscriptions_Product::get_length( $switch_item->product ) ) {
		...
		return apply_filters( 'skyhshoso_switch_proration_extra_to_pay', $extra_to_pay, $switch_item->subscription, $switch_item->cart_item, $switch_item->get_days_in_old_cycle() );
	}
	...
	protected function extend_prepaid_term( $cart_item_key, $switch_item ) {
		...
		$days_to_add = apply_filters( 'skyhshoso_switch_days_to_extend_prepaid_term', $days_to_add, $cart_item_key, $switch_item );
	...
	protected function apportion_length( $switch_item ) {
		$base_length = $switch_item->product->get_meta( 'subscription_base_length_prorated' );
		...
		if ( is_null( $base_length ) ) {
			...
			// A "lazier" way to do the same would have been to call 'SkyHSHOSO_Subscriptions_Product::get_length( $switch_item->canonical_product_id )', but this breaks APFS, and is more expensive performance-wise.
			$base_length = SkyHSHOSO_Subscriptions_Product::get_length( $switch_item->product );
			$switch_item->product->update_meta_data( 'subscription_base_length_prorated', $base_length );
		}
		...
		$length_remaining = apply_filters( 'skyhshoso_switch_length_remaining', $length_remaining, $switch_item );
	...
	public function set_upgrade_cost( $switch_item, $extra_to_pay ) {
		...
		$existing_sign_up_fee = SkyHSHOSO_Subscriptions_Product::get_sign_up_fee( $switch_item->product );
	 ...
	 *
	 * This function is used when a switch results in a negative upgrade cost which typically occurs when stores use the `skyhshoso_switch_proration_switch_type` filter to change the default switch type.
	...
	protected function calculate_fully_reduced_upgrade_cost( $cart_item_key, $switch_item ) {
		...
		$this->cart->cart_contents[ $cart_item_key ]['subscription_switch']['first_payment_timestamp'] = SkyHSHOSO_Subscriptions_Product::get_first_renewal_payment_time( $switch_item->product );
		...
		return ( SkyHSHOSO_Subscriptions_Product::get_price( $switch_item->product ) * $switch_item->cart_item['quantity'] ) - ( $remaining_amount_not_consumed );
	}
