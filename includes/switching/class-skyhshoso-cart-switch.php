...
class SkyHSHOSO_Cart_Switch {
	...
	public function attach_dependant_hooks() {
		// No parent hooks needed — switch cart is independent of renewal cart
	}
	...
	public function maybe_setup_cart() {
		...
		if ( isset( $_GET['pay_for_order'] ) && isset( $_GET['key'] ) && isset( $wp->query_vars['order-pay'] ) && isset( $_GET['subscription_switch'] ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Recommended
			...
			if ( $order->get_meta( 'order_key' ) == $order_key && $order->has_status( array( 'pending', 'failed' ) ) && skyhshoso_order_contains_switch( $order ) ) {
				...
				foreach ( $order->get_items() as $item_id => $line_item ) {
					...
					$order_item = skyhshoso_get_order_item( $item_id, $order );
					$product    = wc_get_product( skyhshoso_get_canonical_product_id( $order_item ) );
	...
	public function order_button_text( $place_order_text ) {
		$cart_switches = SkyHSHOSO_Subscriptions_Switcher::cart_contains_switches();

		if ( isset( WC()->cart ) && $cart_switches && count( $cart_switches ) === count( WC()->cart->get_cart() ) ) {
			$place_order_text = _x( 'Switch subscription', 'The place order button text while switching a subscription', 'skyhs-hosting-solution' );
		}
