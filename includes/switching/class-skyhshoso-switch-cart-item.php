<?php
/**
 * SkyHS Hosting Solution Switch Cart Item.
 *
 ...
 * @since   2.6.0
 */
class SkyHSHOSO_Switch_Cart_Item {
	 ...
	 *
	 * @throws Exception If SkyHSHOSO_Subscriptions_Product::get_expiration_date() returns an invalid date.
	 */
	public function __construct( $cart_item, $subscription, $existing_item ) {
		$this->cart_item               = $cart_item;
		$this->subscription            = $subscription;
		$this->existing_item           = $existing_item;
		$this->canonical_product_id    = skyhshoso_get_canonical_product_id( $cart_item );
		$this->product                 = $cart_item['data'];
		$this->next_payment_timestamp  = $cart_item['subscription_switch']['next_payment_timestamp'];
		$this->end_timestamp           = skyhshoso_date_to_time( SkyHSHOSO_Subscriptions_Product::get_expiration_date( $this->canonical_product_id, $this->subscription->get_date( 'last_order_date_created' ) ) );
	}
	...
	public function get_old_price_per_day() {
		if ( ! isset( $this->old_price_per_day ) ) {
			...
			$this->old_price_per_day = apply_filters( 'skyhshoso_switch_proration_old_price_per_day', $old_price_per_day, $this->subscription, $this->cart_item, $total_paid_for_current_period, $days_in_old_cycle );
		}
	...
	public function get_new_price_per_day() {
		if ( ! isset( $this->new_price_per_day ) ) {
			...
			if ( $this->is_switch_during_trial() && $this->trial_periods_match() ) {
				...
				$new_price_per_day = ( SkyHSHOSO_Subscriptions_Product::get_price( $this->product ) * $this->cart_item['quantity'] ) / $days_in_new_cycle;
			}

			$this->new_price_per_day = apply_filters( 'skyhshoso_switch_proration_new_price_per_day', $new_price_per_day, $this->subscription, $this->cart_item, $days_in_new_cycle );
		}
	...
	public function get_last_order_paid_time() {
		if ( ! isset( $this->last_order_paid_time ) ) {
			$last_order = skyhshoso_get_last_non_early_renewal_order( $this->subscription );
	...
	public function get_total_paid_for_current_period() {
		...
		if ( ! isset( $this->total_paid_for_current_period ) ) {
			...
			if ( $this->is_switch_after_fully_reduced_prepaid_term() ) {
				$this->total_paid_for_current_period = SkyHSHOSO_Subscriptions_Switcher::calculate_total_paid_since_last_order( $this->subscription, $this->existing_item, 'exclude_sign_up_fees', array( $this->get_last_switch_order() ) );
			} else {
				$this->total_paid_for_current_period = SkyHSHOSO_Subscriptions_Switcher::calculate_total_paid_since_last_order( $this->subscription, $this->existing_item, 'exclude_sign_up_fees' );
			}
		}
	...
	public function get_switch_type() {
		if ( ! isset( $this->switch_type ) ) {
			...
			$switch_type = apply_filters( 'skyhshoso_switch_proration_switch_type', $switch_type, $this->subscription, $this->cart_item, $old_price_per_day, $new_price_per_day );

			if ( ! in_array( $switch_type, array( 'upgrade', 'downgrade', 'crossgrade' ) ) ) {
				// translators: placeholder is a switch type.
				throw new UnexpectedValueException( sprintf( esc_html( __( 'Invalid switch type "%s". Switch must be one of: "upgrade", "downgrade" or "crossgrade".', 'skyhs-hosting-solution' ) ), esc_html( $switch_type ) ) );
			}
	...
	public function calculate_days_in_old_cycle() {
		...
		if ( 'days_between_payments' === $method_to_use ) {
			...
			$days_in_old_cycle = skyhshoso_get_days_in_cycle( $this->subscription->get_billing_period(), $this->subscription->get_billing_interval() );
		}

		return apply_filters( 'skyhshoso_switch_proration_days_in_old_cycle', $days_in_old_cycle, $this->subscription, $this->cart_item );
	}
	...
	public function calculate_days_in_new_cycle() {
		...
		$new_billing_period   = SkyHSHOSO_Subscriptions_Product::get_period( $this->product );
		$new_billing_interval = SkyHSHOSO_Subscriptions_Product::get_interval( $this->product );
		...
		$days_in_new_cycle = ( skyhshoso_add_time( $new_billing_interval, $new_billing_period, $last_order_time ) - $last_order_time ) / DAY_IN_SECONDS;
		...
		return apply_filters( 'skyhshoso_switch_proration_days_in_new_cycle', $days_in_new_cycle, $this->subscription, $this->cart_item, $days_in_old_cycle );
	}
	...
	public function trial_periods_match() {
		 ...
		 * We also need to pass the new product's ID so the raw product's trial is used, not the filtered trial set by @see SkyHSHOSO_Subscriptions_Switcher::maybe_unset_free_trial() && SkyHSHOSO_Subscriptions_Switcher::maybe_set_free_trial().
		 */
		$matching_length = (int) SkyHSHOSO_Subscriptions_Product::get_trial_length( $this->product->get_id() ) === (int) SkyHSHOSO_Subscriptions_Product::get_trial_length( $existing_product );
		$matching_period = SkyHSHOSO_Subscriptions_Product::get_trial_period( $this->product->get_id() ) === SkyHSHOSO_Subscriptions_Product::get_trial_period( $existing_product );
	 ...
	 * @see SkyHSHOSO_Switch_Totals_Calculator::reduce_prepaid_term()
	 *
	 * @since 3.0.7
	...
	protected function is_switch_after_fully_reduced_prepaid_term() {
		...
		if ( ! isset( $this->is_switch_after_fully_reduced_prepaid_term ) ) {
			 ...
			 */
			$first_payment_after_switch = SkyHSHOSO_Subscriptions_Product::get_first_renewal_payment_time( $this->existing_item->get_product(), gmdate( 'Y-m-d H:i:s', $switch_paid_date->format( 'U' ) ) );
	...
	public function is_switch_to_one_payment_subscription() {
		return 1 === absint( SkyHSHOSO_Subscriptions_Product::get_length( $this->product ) );
	}
}
