 ...
 * @subpackage SkyHSHOSO_Subscriptions_Switcher
 * @category Class
 * @since 1.4
 */
class SkyHSHOSO_Subscriptions_Switcher {

	/**
	 * The last known switch total calculator instance which was calculated.
	 *
	 * @since 2.6.1
	 * @var SkyHSHOSO_Switch_Totals_Calculator
	 */
	...
	public static function init() {
		...
		// Render "skyhshoso_switching_options" field
		add_action( 'woocommerce_admin_field_skyhshoso_switching_options', __CLASS__ . '::switching_options_field_html' );
		...
		add_action( 'woocommerce_store_api_checkout_update_order_meta', array( __CLASS__, 'process_checkout' ), 50, 1 );
		...
		add_filter( 'skyhshoso_recurring_cart_next_payment_date', array( __CLASS__, 'recurring_cart_next_payment_date' ), 100, 2 );

		// Make sure the new end date starts from the end of the time that has already paid for
		add_filter( 'skyhshoso_recurring_cart_end_date', array( __CLASS__, 'recurring_cart_end_date' ), 100, 3 );
		...
		add_action( 'skyhshoso_recurring_cart_start_date', array( __CLASS__, 'maybe_unset_free_trial' ), 0, 1 );
		add_action( 'skyhshoso_recurring_cart_end_date', array( __CLASS__, 'maybe_set_free_trial' ), 100, 1 );
		...
		// Removed: log_switches action (debug logging removed for production)
		add_filter( 'skyhshoso_admin_subscription_related_orders_to_display', array( __CLASS__, 'display_switches_in_related_order_metabox' ), 10, 3 );
		...
		// Register data store for pending switch items
		add_filter( 'woocommerce_data_stores', array( __CLASS__, 'register_data_stores' ) );
	}

	/**
	 * Debug logger for switch processing.
	 */
	private static function ps_log( $message ) {
	}

	/**
	 * Register custom data stores for SkyHS switcher items.
	 * 
	 * @param array $stores
	 * @return array
	 */
	public static function register_data_stores( $stores ) {
		...
		$stores['order-item-fee_pending_switch'] = 'WC_Order_Item_Fee_Data_Store';
		$stores['order-item-coupon_pending_switch'] = 'WC_Order_Item_Coupon_Data_Store';
		return $stores;
	}
	...
	public static function subscription_switch_handler() {
		...
		if ( isset( $_GET['switch-subscription'] ) && isset( $_GET['item'] ) ) {
			...
			$subscription = skyhshoso_get_subscription( absint( $_GET['switch-subscription'] ) ); // phpcs:ignore WordPress.Security.NonceVerification.Recommended
			$line_item    = skyhshoso_get_order_item( absint( $_GET['item'] ), $subscription ); // phpcs:ignore WordPress.Security.NonceVerification.Recommended

			// Visiting a switch link for someone elses subscription or if the switch link doesn't contain a valid nonce
			if ( ! is_object( $subscription ) || empty( $_GET['_skyhshoso_nonce'] ) || ! wp_verify_nonce( sanitize_text_field( wp_unslash( $_GET['_skyhshoso_nonce'] ) ), 'skyhshoso_switch_request' ) || empty( $line_item ) || ! self::can_item_be_switched_by_user( $line_item, $subscription ) ) {

				wp_redirect( remove_query_arg( array( 'switch-subscription', 'auto-switch', 'item', '_skyhshoso_nonce' ) ) );
				exit();

			} else {

				if ( isset( $_GET['auto-switch'] ) ) {
					$switch_message = __( 'You have a subscription to this product. Choosing a new subscription will replace your existing subscription.', 'skyhs-hosting-solution' );
				} else {
					$switch_message = __( 'Choose a new subscription.', 'skyhs-hosting-solution' );
				}
			...
			foreach ( $switch_items as $cart_item_key => $switch_item ) {
				...
				$subscription  = skyhshoso_get_subscription( $switch_item['subscription_id'] );
				...
				if ( $is_valid_item ) {
					if ( empty( $switch_item['item_id'] ) ) {
						...
						$item = skyhshoso_get_order_item( $switch_item['item_id'], $subscription );
			...
			if ( $removed_item_count > 0 ) {
				wc_add_notice( _n( 'Your cart contained an invalid subscription switch request. It has been removed.', 'Your cart contained invalid subscription switch requests. They have been removed.', $removed_item_count, 'skyhs-hosting-solution' ), 'error' );
			...
			if ( $product->is_type( 'grouped' ) ) { // If we're on a grouped product's page, we need to check if this grouped product has children which are limited and may need to be switched
				...
				foreach ( $child_ids as $child_id ) {
					...
					if ( 'no' != skyhshoso_get_product_limitation( $product ) && skyhshoso_is_product_switchable_type( $product ) ) {
						$limited_switchable_products[] = $product;
					}
				}
			} elseif ( 'no' != skyhshoso_get_product_limitation( $product ) && skyhshoso_is_product_switchable_type( $product ) ) {
			...
			if ( ! empty( $limited_switchable_products ) ) {
				...
				$subscriptions = skyhshoso_get_users_subscriptions( get_current_user_id() );

				foreach ( $subscriptions as $subscription ) {
					foreach ( $limited_switchable_products as $product ) {

						if ( (int) $subscription->get_product_id() !== (int) $product->get_id() ) {
							continue;
						}

						$limitation = skyhshoso_get_product_limitation( $product );

						if ( 'any' == $limitation || $subscription->has_status( $limitation ) ) {

							$subscribed_notice = __( 'You have already subscribed to this product and it is limited to one per customer. You can not purchase the product again.', 'skyhs-hosting-solution' );
							...
							if ( $subscription->needs_payment() ) {
								...
								if ( $last_order->needs_payment() ) {
									...
									$subscribed_notice = sprintf( __( '%1$s Complete payment on %2$sOrder %3$s%4$s to be able to change your subscription.', 'skyhs-hosting-solution' ), $subscribed_notice, sprintf( '<a href="%s">', esc_url( $last_order->get_checkout_payment_url() ) ), $last_order->get_order_number(), '</a>' );
								}
								...
								if ( apply_filters( 'skyhshoso_initiate_auto_switch', self::can_item_be_switched_by_user( $item, $subscription ), $item, $subscription ) ) {
	...
	public static function add_switch_query_arg_grouped( $permalink ) {
		...
		if ( isset( $_GET['switch-subscription'] ) && isset( $_GET['item'] ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Recommended
			$permalink = self::add_switch_query_args( absint( wp_unslash( $_GET['switch-subscription'] ) ), absint( wp_unslash( $_GET['item'] ) ), $permalink ); // phpcs:ignore WordPress.Security.NonceVerification.Recommended
		}
	...
	public static function add_switch_query_arg_post_link( $permalink, $post ) {
		if ( ! isset( $_GET['switch-subscription'] ) || ! isset( $_GET['item'] ) || ! is_main_query() || ! is_product() || 'product' !== $post->post_type ) { // phpcs:ignore WordPress.Security.NonceVerification.Recommended
			return $permalink;
		}
		...
		switch ( $type ) {
			...
			case 'subscription':
				return self::add_switch_query_args( absint( wp_unslash( $_GET['switch-subscription'] ) ), absint( wp_unslash( $_GET['item'] ) ), $permalink ); // phpcs:ignore WordPress.Security.NonceVerification.Recommended
	...
	public static function add_settings( $settings ) {
		...
		$switching_settings = array(
			array(
				'name' => __( 'Switching', 'skyhs-hosting-solution' ),
				'type' => 'title',
				// translators: placeholders are opening and closing link tags
				'desc' => sprintf( __( 'Allow subscribers to switch (upgrade or downgrade) between different subscriptions. %1$sLearn more%2$s.', 'skyhs-hosting-solution' ), '<a href="' . esc_url( 'http://docs.woocommerce.com/document/subscriptions/switching-guide/' ) . '">', '</a>' ),
				'id'   => 'skyhshoso' . '_switch_settings',
			),
			array(
				'type' => 'skyhshoso_switching_options',
				'id'   => 'skyhshoso' . '_allow_switching',
			),
			array(
				'name'     => __( 'Prorate Recurring Payment', 'skyhs-hosting-solution' ),
				'desc'     => __( 'When switching to a subscription with a different recurring payment or billing period, should the price paid for the existing billing period be prorated when switching to the new subscription?', 'skyhs-hosting-solution' ),
				'tip'      => '',
				'id'       => 'skyhshoso' . '_apportion_recurring_price',
				'css'      => 'min-width:150px;',
				'default'  => 'no',
				'type'     => 'select',
				'class'    => 'wc-enhanced-select',
				'options'  => array(
					'no'              => _x( 'Never', 'when to allow a setting', 'skyhs-hosting-solution' ),
					'virtual-upgrade' => _x( 'For Upgrades of Virtual Subscription Products Only', 'when to prorate recurring fee when switching', 'skyhs-hosting-solution' ),
					'yes-upgrade'     => _x( 'For Upgrades of All Subscription Products', 'when to prorate recurring fee when switching', 'skyhs-hosting-solution' ),
					'virtual'         => _x( 'For Upgrades & Downgrades of Virtual Subscription Products Only', 'when to prorate recurring fee when switching', 'skyhs-hosting-solution' ),
					'yes'             => _x( 'For Upgrades & Downgrades of All Subscription Products', 'when to prorate recurring fee when switching', 'skyhs-hosting-solution' ),
				),
				'desc_tip' => true,
			),
			array(
				'name'     => __( 'Prorate Sign up Fee', 'skyhs-hosting-solution' ),
				'desc'     => __( 'When switching to a subscription with a sign up fee, you can require the customer pay only the gap between the existing subscription\'s sign up fee and the new subscription\'s sign up fee (if any).', 'skyhs-hosting-solution' ),
				'tip'      => '',
				'id'       => 'skyhshoso' . '_apportion_sign_up_fee',
				'css'      => 'min-width:150px;',
				'default'  => 'no',
				'type'     => 'select',
				'class'    => 'wc-enhanced-select',
				'options'  => array(
					'no'   => _x( 'Never (do not charge a sign up fee)', 'when to prorate signup fee when switching', 'skyhs-hosting-solution' ),
					'full' => _x( 'Never (charge the full sign up fee)', 'when to prorate signup fee when switching', 'skyhs-hosting-solution' ),
					'yes'  => _x( 'Always', 'when to prorate signup fee when switching', 'skyhs-hosting-solution' ),
				),
				'desc_tip' => true,
			),
			array(
				'name'     => __( 'Prorate Subscription Length', 'skyhs-hosting-solution' ),
				'desc'     => __( 'When switching to a subscription with a length, you can take into account the payments already completed by the customer when determining how many payments the subscriber needs to make for the new subscription.', 'skyhs-hosting-solution' ),
				'tip'      => '',
				'id'       => 'skyhshoso' . '_apportion_length',
				'css'      => 'min-width:150px;',
				'default'  => 'no',
				'type'     => 'select',
				'class'    => 'wc-enhanced-select',
				'options'  => array(
					'no'      => _x( 'Never', 'when to allow a setting', 'skyhs-hosting-solution' ),
					'virtual' => _x( 'For Virtual Subscription Products Only', 'when to prorate first payment / subscription length', 'skyhs-hosting-solution' ),
					'yes'     => _x( 'For All Subscription Products', 'when to prorate first payment / subscription length', 'skyhs-hosting-solution' ),
				),
				'desc_tip' => true,
			),
			array(
				'name'     => __( 'Switch Button Text', 'skyhs-hosting-solution' ),
				'desc'     => __( 'Customise the text displayed on the button next to the subscription on the subscriber\'s account page. The default is "Switch Subscription", but you may wish to change this to "Upgrade" or "Change Subscription".', 'skyhs-hosting-solution' ),
				'tip'      => '',
				'id'       => 'skyhshoso' . '_switch_button_text',
				'css'      => 'min-width:150px;',
				'default'  => __( 'Upgrade or Downgrade', 'skyhs-hosting-solution' ),
				'type'     => 'text',
				'desc_tip' => true,
			),
			array(
	...
	/**
	 * Render the skyhshoso_switching_options setting field.
	 *
	 * @since 2.6.0
	 * @param array $data
	 */
	public static function switching_options_field_html( $data ) {
		...
		<tr valign="top">
			<th scope="row" class="titledesc">
				<label for="skyhshoso_switching_options">
					<?php esc_html_e( 'Allow Switching', 'skyhs-hosting-solution' ) ?>
				</label>
			</th>
			<td class="forminp forminp-skyhshoso_switching_options">
				<div class="skyhshoso_setting_switching_options" id="woocommerce_subscriptions_allow_switching">
					<label>
						<input <?php checked( $allow_switching_variable_checked ); ?> type="checkbox" name="<?php echo esc_attr( 'skyhshoso' . '_allow_switching_variable' ) ?>"/>
						<?php echo esc_html_x( 'Between Subscription Variations', 'when to allow switching', 'skyhs-hosting-solution' ); ?>
					</label>
					<label>
						<input <?php checked( $allow_switching_grouped_checked ); ?> type="checkbox" name="<?php echo esc_attr( 'skyhshoso' . '_allow_switching_grouped' ) ?>"/>
						<?php echo esc_html_x( 'Between Grouped Subscriptions', 'when to allow switching', 'skyhs-hosting-solution' ); ?>
					</label>
					<?php
					...
					foreach ( $extra_switching_options as $option ) {
						...
						$label = $option['label'];
						$name  = 'skyhshoso' . '_allow_switching_' . $option['id'];
						...
						echo isset( $option['desc_tip'] ) ? wp_kses_post( wc_help_tip( $option['desc_tip'], true ) ) : '';
						echo '</label>';
					}
					?>
				</div>
	...
	public static function print_switch_link( $item_id, $item, $subscription ) {
		...
		if ( skyhshoso_is_order( $subscription ) || 'shop_subscription' !== $subscription->get_type() || ! self::can_item_be_switched_by_user( $item, $subscription ) ) {
			return;
		}

		$switch_url     = esc_url( self::get_switch_url( $item_id, $item, $subscription ) );
		$switch_text    = apply_filters( 'woocommerce_subscriptions_switch_link_text', get_option( 'skyhshoso' . '_switch_button_text', __( 'Upgrade or Downgrade', 'skyhs-hosting-solution' ) ), $item_id, $item, $subscription );
		$switch_classes = apply_filters( 'woocommerce_subscriptions_switch_link_classes', array( 'skyhshoso-switch-link', 'button' ), $item_id, $item, $subscription );
	...
	public static function get_switch_url( $item_id, $item, $subscription ) {
		...
		if ( ! is_object( $subscription ) ) {
			$subscription = skyhshoso_get_subscription( $subscription );
		}

		$product               = wc_get_product( $item['product_id'] );
		$parent_products       = SkyHSHOSO_Subscriptions_Product::get_visible_grouped_parent_product_ids( $product );
		...
		if ( ! empty( $parent_products ) ) {
			...
			if ( ! empty( $_GET ) && is_product() ) { // phpcs:ignore WordPress.Security.NonceVerification.Recommended
				...
				foreach ( $product->get_variation_attributes() as $attribute => $value ) {
					$product_variations[ skyhshoso_maybe_prefix_key( strtolower( $attribute ), 'attribute_' ) ] = $value;
				}

				$additional_query_args = array_intersect_key( $_GET, $product_variations ); // phpcs:ignore WordPress.Security.NonceVerification.Recommended
			}
		}
	...
	protected static function add_switch_query_args( $subscription_id, $item_id, $permalink, $additional_query_args = array() ) {
		...
		$query_args = array_merge(
			...
			array(
				...
				'item'                => absint( $item_id ),
				'_skyhshoso_nonce'           => wp_create_nonce( 'skyhshoso_switch_request' ),
			)
		);
	...
	protected static function is_action_allowed( $action, $item, $subscription = null ) {
		...
		$product_id = skyhshoso_get_canonical_product_id( $item );

		if ( 'switch' === $action ) {
			if ( 'line_item' == $item['type'] && skyhshoso_is_product_switchable_type( $product_id ) ) {
	...
	public static function add_order_meta( $order_id, $posted = array() ) {
		...
		// delete all the existing subscription switch links before adding new ones
				$order->delete_meta_data( '_skyhshoso_switch_subscription' );

		$switches = self::cart_contains_switches( 'any' );

		if ( false !== $switches ) {
			foreach ( $switches as $cart_item_key => $switch_details ) {
				$sub = skyhshoso_get_subscription( $switch_details['subscription_id'] );
				if ( $sub ) {
					$existing = $order->get_meta( '_skyhshoso_switch_subscription' );
					if ( ! is_array( $existing ) ) {
						$existing = $existing ? array( $existing ) : array();
					}
					if ( ! in_array( $sub->get_id(), $existing ) ) {
						$existing[] = $sub->get_id();
						$order->update_meta_data( '_skyhshoso_switch_subscription', $existing );
					}
				}

				// Also add metadata to the corresponding order line item to ensure handle_order_paid knows it is a switch
				foreach ( $order->get_items() as $item_id => $order_item ) {
					$cart_item = isset( WC()->cart->cart_contents[ $cart_item_key ] ) ? WC()->cart->cart_contents[ $cart_item_key ] : null;
					if ( $cart_item && $order_item->get_product_id() == $cart_item['product_id'] && $order_item->get_variation_id() == $cart_item['variation_id'] ) {
						$sign_up_fee_prorated = $cart_item['data']->get_meta( '_subscription_sign_up_fee_prorated', true );
						$price_prorated       = $cart_item['data']->get_meta( '_subscription_price_prorated', true );
						$order_item->update_meta_data( '_switched_subscription_sign_up_fee_prorated', empty( $sign_up_fee_prorated ) ? 0 : $sign_up_fee_prorated );
						$order_item->update_meta_data( '_switched_subscription_price_prorated', empty( $price_prorated ) ? 0 : $price_prorated );
						$order_item->save();
					}
				}
			}
	...
	public static function add_order_item_meta( $order_item_id, $cart_item, $cart_item_key ) {
		...
		if ( false === version_compare( WC()->version, '3.0', '<' ) ) {
			...
			if ( $switches = self::cart_contains_switches() ) {
				foreach ( $switches as $switch_item_key => $switch_details ) {
					if ( $cart_item_key == $switch_item_key ) {
						wc_add_order_item_meta( $order_item_id, '_switched_subscription_sign_up_fee_prorated', WC()->cart->cart_contents[ $cart_item_key ]['data']->get_meta( 'subscription_sign_up_fee_prorated' ) ?: 0, true );
						wc_add_order_item_meta( $order_item_id, '_switched_subscription_price_prorated', WC()->cart->cart_contents[ $cart_item_key ]['data']->get_meta( 'subscription_price_prorated' ) ?: 0, true );
					}
				}
			}
	...
	/**
	 * Get simulated recurring carts for subscription switches.
	 *
	 * @return array
	 */
	public static function get_recurring_carts() {
		$recurring_carts = array();
		if ( ! isset( WC()->cart ) || ! is_object( WC()->cart ) ) {
			return $recurring_carts;
		}

		foreach ( WC()->cart->get_cart() as $cart_item_key => $cart_item ) {
			if ( isset( $cart_item['subscription_switch']['subscription_id'] ) ) {
				$sub_id = $cart_item['subscription_switch']['subscription_id'];
				if ( ! isset( $recurring_carts[ $sub_id ] ) ) {
					$recurring_carts[ $sub_id ] = clone WC()->cart;
					$recurring_carts[ $sub_id ]->cart_contents = array(); // clear items
					$recurring_carts[ $sub_id ]->next_payment_date = 0;
					$recurring_carts[ $sub_id ]->end_date = 0;
				}
				$recurring_carts[ $sub_id ]->cart_contents[ $cart_item_key ] = $cart_item;
			}
		}
	...
	public static function process_checkout( $order_id, $posted_data = array() ) {
		self::ps_log( "process_checkout: order_id=$order_id" );

		$order = wc_get_order( $order_id );
		if ( ! $order ) {
			self::ps_log( "Order #$order_id not found" );
			return;
		}

		$sub_ids = $order->get_meta( '_skyhshoso_switch_subscription' );
		if ( empty( $sub_ids ) ) {
			self::ps_log( "No _skyhshoso_switch_subscription meta on order, not a switch order" );
			return;
		}
		...
		$sub_id = (int) reset( $sub_ids );
		self::ps_log( "Switch subscription ID: $sub_id" );

		$subscription = skyhshoso_get_subscription( $sub_id );
		if ( ! $subscription ) {
			self::ps_log( "Subscription #$sub_id not found" );
			return;
		}

		// Build switch data from the order items and the cart contents
		$switch_order_data = array();
		$switch_order_data[ $subscription->get_id() ] = array(
			'switches' => array(),
		);

		$switch_items = self::cart_contains_switches( 'any' );

		foreach ( $order->get_items() as $order_item_id => $order_item ) {
			$matched = false;

			if ( false !== $switch_items ) {
				foreach ( $switch_items as $cart_item_key => $switch_detail ) {
					$cart_item = WC()->cart->cart_contents[ $cart_item_key ] ?? null;
					if ( ! $cart_item ) {
						continue;
					}
					if ( $order_item->get_product_id() == $cart_item['product_id'] && $order_item->get_variation_id() == $cart_item['variation_id'] ) {
						$switched_item_data = array(
							'add_line_item'    => $order_item_id,
							'remove_line_item' => $switch_detail['item_id'] ?? 0,
						);
						$switch_order_data[ $subscription->get_id() ]['switches'][ $order_item_id ] = $switched_item_data;
						$matched = true;
						self::ps_log( "Matched order item #$order_item_id to switch: " . wp_json_encode( $switched_item_data ) );
						break;
					}
				}
			}
			...
			if ( ! $matched ) {
				// Fallback: add every order item as a switch item
				$switched_item_data = array(
					'add_line_item'    => $order_item_id,
					'remove_line_item' => 0,
				);
				$switch_order_data[ $subscription->get_id() ]['switches'][ $order_item_id ] = $switched_item_data;
				self::ps_log( "Fallback: order item #$order_item_id added as switch" );
			}
		}

		self::ps_log( "Switch order data: " . wp_json_encode( $switch_order_data ) );
		$order->update_meta_data( 'subscription_switch_data', $switch_order_data );
		$order->save();

		self::ps_log( "Saved subscription_switch_data to order #$order_id" );
	...
	public static function update_shipping_methods( $subscription, $recurring_cart ) {
		skyhshoso_deprecated_function( __METHOD__, '4.8.0', 'The use of this function is no longer recommended and will be removed in a future version.' );

		// Shipping handling - only applicable for WC_Subscription objects
		...
		foreach ( $subscription->get_shipping_methods() as $shipping_method_id => $shipping_method ) {
			skyhshoso_update_order_item_type( $shipping_method_id, 'shipping_switched', $subscription->get_id() );
		}

		// Then zero the order_shipping total so we have a clean slate to add to
		$subscription->set_total_shipping( 0 );

			// SkyHS handles shipping - no-op
	...
	public static function cart_contains_switches( $item_action = 'switch' ) {
		...
		foreach ( WC()->cart->cart_contents as $cart_item_key => $cart_item ) {
			...
			if ( ! skyhshoso_get_subscription( $cart_item['subscription_switch']['subscription_id'] ) ) {
				WC()->cart->remove_cart_item( $cart_item_key );
				wc_add_notice( __( 'Your cart contained an invalid subscription switch request. It has been removed.', 'skyhs-hosting-solution' ), 'error' );
				continue;
			}
	...
	public static function cart_contains_switch_for_product( $product ) {
		...
		if ( false !== $switch_items ) {
			...
			foreach ( $switch_items as $switch_item_details ) {
				...
				$switch_product  = wc_get_product( skyhshoso_get_order_items_product_id( $switch_item_details['item_id'] ) );
				$parent_products = SkyHSHOSO_Subscriptions_Product::get_parent_ids( $product );
	...
	public static function trigger_switch_added_to_cart_hook( $cart_item_key, $product_id, $quantity, $variation_id, $variation_attributes, $cart_item_data ) {
		...
		$subscription  = skyhshoso_get_subscription( $cart_item_data['subscription_switch']['subscription_id'] );
		$existing_item = skyhshoso_get_order_item( $cart_item_data['subscription_switch']['item_id'], $subscription );
	...
	public static function retain_coupons( $subscription ) {
		foreach ( skyhshoso_get_used_coupon_codes( $subscription ) as $coupon_code ) {
	...
	public static function validate_switch_request( $is_valid, $product_id, $quantity, $variation_id = '' ) {
		...
		try {
			...
			if ( empty( $_GET['_skyhshoso_nonce'] ) || ! wp_verify_nonce( sanitize_text_field( wp_unslash( $_GET['_skyhshoso_nonce'] ) ), 'skyhshoso_switch_request' ) ) {
				return false;
			}

			$subscription = skyhshoso_get_subscription( absint( $_GET['switch-subscription'] ) );
			$item_id      = isset( $_GET['item'] ) ? absint( wp_unslash( $_GET['item'] ) ) : 0;
			$item         = skyhshoso_get_order_item( $item_id, $subscription );

			// Prevent switching to non-subscription product
			if ( ! SkyHSHOSO_Subscriptions_Product::is_subscription( $product_id ) ) {
				throw new Exception( __( 'You can only switch to a subscription product.', 'skyhs-hosting-solution' ) );
			}
			...
			if ( empty( $item ) ) {
				...
				throw new Exception( __( 'We can not find your old subscription item.', 'skyhs-hosting-solution' ) );
				...
				if ( $is_identical_product ) {
					throw new Exception( __( 'You can not switch to the same subscription.', 'skyhs-hosting-solution' ) );
				}
	...
	public static function set_switch_details_in_cart( $cart_item_data, $product_id, $variation_id ) {
		...
		try {
			if ( ! isset( $_GET['switch-subscription'] ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Recommended
			...
			if ( ! current_user_can( 'switch_shop_subscription', $subscription->get_id() ) ) {
				wc_add_notice( __( 'You can not switch this subscription. It appears you do not own the subscription.', 'skyhs-hosting-solution' ), 'error' );
				WC()->cart->empty_cart( true );
				wp_redirect( get_permalink( $product_id ) );
				exit();
			}

			$item_id = isset( $_GET['item'] ) ? absint( wp_unslash( $_GET['item'] ) ) : 0; // phpcs:ignore WordPress.Security.NonceVerification.Recommended
			$item = skyhshoso_get_order_item( $item_id, $subscription ); // phpcs:ignore WordPress.Security.NonceVerification.Recommended
			...
			$cart_item_data['subscription_switch'] = array(
				...
				'item_id'                => $item_id, // phpcs:ignore WordPress.Security.NonceVerification.Recommended
			...
			wc_add_notice( __( 'There was an error locating the switch details.', 'skyhs-hosting-solution' ), 'error' );
			WC()->cart->empty_cart( true );
	...
	public static function subscription_items_sign_up_fee( $sign_up_fee, $line_item, $subscription, $tax_inclusive_or_exclusive = '' ) {
		...
		foreach ( skyhshoso_get_switch_orders_for_subscription( $subscription->get_id() ) as $order ) {
			foreach ( $order->get_items() as $order_item_id => $order_item ) {
				if ( skyhshoso_get_canonical_product_id( $line_item ) == skyhshoso_get_canonical_product_id( $order_item ) ) {
					...
					if ( 'inclusive_of_tax' == $tax_inclusive_or_exclusive && $order->get_prices_include_tax() ) {
	...
	public static function calculate_prorated_totals( $cart ) {
		if ( self::cart_contains_switches( 'any' ) ) {
			self::$switch_totals_calculator = new SkyHSHOSO_Switch_Totals_Calculator( $cart );
	...
	public static function recurring_cart_next_payment_date( $first_renewal_date, $cart ) {
		...
		foreach ( $cart->get_cart() as $cart_item_key => $cart_item ) {
			if ( isset( $cart_item['subscription_switch']['first_payment_timestamp'] ) && 0 != $cart_item['subscription_switch']['first_payment_timestamp'] ) {
				$first_renewal_date = ( '1' != SkyHSHOSO_Subscriptions_Product::get_length( $cart_item['data'] ) ) ? gmdate( 'Y-m-d H:i:s', $cart_item['subscription_switch']['first_payment_timestamp'] ) : 0;
			}
		}
		...
		if ( 0 !== $end_date ) {
			foreach ( $cart->get_cart() as $cart_item_key => $cart_item ) {
				...
				if ( isset( $cart_item['subscription_switch']['subscription_id'] ) && isset( $cart_item['data'] ) && $product == $cart_item['data'] ) {
					...
					if ( 1 == SkyHSHOSO_Subscriptions_Product::get_length( $cart_item['data'] ) && 0 !== $next_payment_time && isset( $cart_item['subscription_switch']['recurring_payment_prorated'] ) ) {
					...
					} elseif ( 0 !== $next_payment_time && SkyHSHOSO_Subscriptions_Product::get_length( $cart_item['data'] ) > 1 ) {
						// remove trial period on the switched subscription when calculating the new end date
						$trial_length = $cart_item['data']->get_meta( 'subscription_trial_length' );
						$cart_item['data']->update_meta_data( 'subscription_trial_length', 0 );

						$end_date = SkyHSHOSO_Subscriptions_Product::get_expiration_date( $cart_item['data'], gmdate( 'Y-m-d H:i:s', $next_payment_time ) );

						// add back the trial length if it has been spoofed
						$cart_item['data']->update_meta_data( 'subscription_trial_length', $trial_length );
	...
	public static function get_recurring_cart_key( $cart_key, $cart_item ) {
		...
		if ( isset( $cart_item['subscription_switch']['first_payment_timestamp'] ) ) {
			...
			$cart_key = gmdate( 'Y_m_d', $cart_item['subscription_switch']['first_payment_timestamp'] );
	...
	public static function customise_product_string_inclusions( $inclusions, $product ) {
		...
		if ( isset( $_GET['switch-subscription'] ) || self::cart_contains_switch_for_product( $product ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Recommended

			$inclusions['trial_length'] = false;

			$apportion_length      = get_option( 'skyhshoso' . '_apportion_length', 'no' );
			$apportion_sign_up_fee = get_option( 'skyhshoso' . '_apportion_sign_up_fee', 'no' );
	...
	public static function remove_renewal_order_meta_query( $order_meta_query ) {
		_deprecated_function( __METHOD__, '4.7.0', 'SkyHSHOSO_Subscriptions_Switcher::remove_renewal_order_meta' );
	...
	public static function addons_add_to_cart_url( $add_to_cart_url ) {
		...
		if ( isset( $_GET['switch-subscription'] ) && isset( $_GET['item'] ) && false === strpos( $add_to_cart_url, 'switch-subscription' ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Recommended
			$add_to_cart_url = self::add_switch_query_args( absint( wp_unslash( $_GET['switch-subscription'] ) ), absint( wp_unslash( $_GET['item'] ) ), $add_to_cart_url ); // phpcs:ignore WordPress.Security.NonceVerification.Recommended
		}
	 ...
	 *
	 * Commits all the changes calculated and saved by @see SkyHSHOSO_Subscriptions_Switcher::process_checkout(), updating subscription
	...
	public static function process_subscription_switches( $order_id, $order_old_status, $order_new_status ) {
		self::ps_log( "process_subscription_switches: order_id=$order_id, old_status=$order_old_status, new_status=$order_new_status" );
		...
		$switch_processed = $order->get_meta( 'completed_subscription_switch' );
		$contains_switch  = skyhshoso_order_contains_switch( $order );

		self::ps_log( "Order #$order_id: contains_switch=" . ( $contains_switch ? 'yes' : 'no' ) . ", already_processed=" . ( $switch_processed ?: 'no' ) );

		if ( ! $contains_switch || 'true' == $switch_processed ) {
			return;
		}
		...
		self::ps_log( "Order #$order_id: order_completed=" . ( $order_completed ? 'yes' : 'no' ) );
		...
		if ( $order_completed ) {
			try {
				...
				$order->update_meta_data( 'completed_subscription_switch', 'true' );
	...
	public static function is_product_of_switchable_type( $product ) {
		...
		_deprecated_function( __METHOD__, '2.0.7', 'skyhshoso_is_product_switchable_type' );

		$allow_switching = false;
		$switch_setting  = get_option( 'skyhshoso' . '_allow_switching', 'no' );

		// does the current switch setting allow switching for variable or variable_grouped
		if ( 'variable_grouped' == $switch_setting || ( $product->is_type( array( 'variable-subscription', 'subscription_variation' ) ) && 'variable' == $switch_setting ) || ( 'grouped' == $switch_setting && ( $product->is_type( 'grouped' ) || $product->get_parent_id() ) ) ) {
	...
	public static function hidden_order_itemmeta( $hidden_meta_keys ) {
		...
		if ( apply_filters( 'woocommerce_subscriptions_hide_switch_itemmeta', ! defined( 'SKYHSHOSO_DEBUG' ) || true !== SKYHSHOSO_DEBUG ) ) {
	...
	public static function add_cart_item_switch_direction( $product_subtotal, $cart_item, $cart_item_key ) {
		...
		if ( ! empty( $cart_item['subscription_switch'] ) ) {
			...
			switch ( $cart_item['subscription_switch']['upgraded_or_downgraded'] ) {
				case 'downgraded':
					$direction = _x( 'Downgrade', 'a switch type', 'skyhs-hosting-solution' );
					break;
				case 'upgraded':
					$direction = _x( 'Upgrade', 'a switch type', 'skyhs-hosting-solution' );
					break;
				default:
					$direction = _x( 'Crossgrade', 'a switch type', 'skyhs-hosting-solution' );
				break;
			}

			// translators: %1: product subtotal, %2: HTML span tag, %3: direction (upgrade, downgrade, crossgrade), %4: closing HTML span tag
			$product_subtotal = sprintf( _x( '%1$s %2$s(%3$s)%4$s', 'product subtotal string', 'skyhs-hosting-solution' ), $product_subtotal, '<span class="subscription-switch-direction">', $direction, '</span>' );
	...
	public static function maybe_add_switched_callback( $order ) {
		if ( skyhshoso_order_contains_switch( $order ) ) {
			...
			foreach ( $subscriptions as $subscription ) {
				foreach ( $subscription->get_items() as $new_order_item ) {
					if ( isset( $new_order_item['switched_subscription_item_id'] ) ) {
						$product_id = skyhshoso_get_canonical_product_id( $new_order_item );
						...
						foreach ( $order->get_items() as $order_item ) {
							if ( skyhshoso_get_canonical_product_id( $order_item ) == $product_id ) {
								do_action( 'woocommerce_subscriptions_switched_item', $subscription, $new_order_item, WC_Order_Factory::get_order_item( $new_order_item['switched_subscription_item_id'] ) );
								break;
							}
						}
					}
	...
	*/
	public static function remove_download_permissions_after_switch( $subscription, $new_item, $old_item ) {
		...
		$product_id = skyhshoso_get_canonical_product_id( $old_item );
		// phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching
		$wpdb->delete( $wpdb->prefix . 'woocommerce_downloadable_product_permissions', array( 'product_id' => $product_id, 'order_id' => $subscription->get_id(), 'user_id' => $subscription->get_user_id() ), array( '%d', '%d', '%d' ) );
	 ...
	 *
	 * Performs all the changes calculated and saved by @see SkyHSHOSO_Subscriptions_Switcher::process_checkout(), updating subscription
	 * line items, schedule, dates and totals to reflect the changes made in this switch order.
	 *
	 * @param WC_Order $order
	 * @since 2.1
	...
	public static function complete_subscription_switches( $order ) {
		...
		// Get the switch meta
		$switch_order_data = $order->get_meta( 'subscription_switch_data' );
		...
		foreach ( $switch_order_data as $subscription_id => $switch_data ) {
			...
			if ( ! empty( $switch_data['switches'] ) && is_array( $switch_data['switches'] ) ) {
				foreach ( $switch_data['switches'] as $order_item_id => $switched_item_data ) {
					...
					$switch_order_item        = skyhshoso_get_order_item( $order_item_id, $order );

					// If we are adding a line item to an existing subscription...
					if ( $add_subscription_item ) {
						skyhshoso_update_order_item_type( $switched_item_data['add_line_item'], 'line_item', $subscription->get_id() );

						// ACTUAL product update for SkyHS!
						if ( $switch_order_item ) {
							$new_product_id = $switch_order_item->get_variation_id() ? $switch_order_item->get_variation_id() : $switch_order_item->get_product_id();
							$new_product    = wc_get_product( $new_product_id );
							$new_price      = $new_product ? (float) $new_product->get_price() : 0.0;

							SkyHSHOSO_Subscription_DB::update( $subscription->get_id(), array(
								'product_id'   => $switch_order_item->get_product_id(),
								'variation_id' => $switch_order_item->get_variation_id(),
								'amount'       => $new_price,
							) );
						}
					...
					if ( $remove_subscription_item ) {
						$old_subscription_item = skyhshoso_get_order_item( $switched_item_data['remove_line_item'], $subscription );
					}

					if ( $remove_subscription_item && empty( $old_subscription_item ) ) {
						throw new Exception( esc_html( __( 'The original subscription item being switched cannot be found.', 'skyhs-hosting-solution' ) ) );
					} elseif ( empty( $switch_order_item ) ) {
						throw new Exception( esc_html( __( 'The item on the switch order cannot be found.', 'skyhs-hosting-solution' ) ) );
					}
					...
					$new_item_name = skyhshoso_get_order_item_name( $switch_order_item, array( 'attributes' => true ) );

					if ( $remove_subscription_item ) {
						skyhshoso_update_order_item_type( $switched_item_data['remove_line_item'], 'line_item_switched', $subscription->get_id() );

						// translators: 1$: old item, 2$: new item when switching
						$add_note = sprintf( _x( 'Customer switched from: %1$s to %2$s.', 'used in order notes', 'skyhs-hosting-solution' ), skyhshoso_get_order_item_name( $old_subscription_item, array( 'attributes' => true ) ), $new_item_name );
					} else {
						// translators: %s: new item name.
						$add_note = sprintf( _x( 'Customer added %s.', 'used in order notes', 'skyhs-hosting-solution' ), $new_item_name );
					}
			...
			foreach ( $subscription->get_items( 'coupon' ) as $coupon_id => $coupon ) {
				skyhshoso_update_order_item_type( $coupon_id, 'coupon_switched', $subscription->get_id() );
			}
			...
			if ( ! empty( $switch_data['coupons'] ) && is_array( $switch_data['coupons'] ) ) {
				...
				foreach ( $switch_data['coupons'] as $coupon_code ) {
					skyhshoso_update_order_item_type( $coupon_code, 'coupon', $subscription->get_id() );
				}
			}

			// Archive the old fees
			foreach ( $subscription->get_fees() as $fee_item_id => $fee ) {
				skyhshoso_update_order_item_type( $fee_item_id, 'fee_switched', $subscription->get_id() );
			}
			...
			if ( ! empty( $switch_data['fee_items'] ) && is_array( $switch_data['fee_items'] ) ) {
				...
				foreach ( $switch_data['fee_items'] as $fee_item_id ) {
					skyhshoso_update_order_item_type( $fee_item_id, 'fee', $subscription->get_id() );
				}
			}
			...
			if ( ! empty( $switch_data['shipping_line_items'] ) && is_array( $switch_data['shipping_line_items'] ) ) {
				...
				foreach ( $subscription->get_shipping_methods() as $shipping_line_item_id => $item ) {
					skyhshoso_update_order_item_type( $shipping_line_item_id, 'shipping_switched', $subscription->get_id() );
				}

				// Flip the switched shipping line items "on"
				foreach ( $switch_data['shipping_line_items'] as $shipping_line_item_id ) {
					skyhshoso_update_order_item_type( $shipping_line_item_id, 'shipping', $subscription->get_id() );
				}
			}
			...
			skyhshoso_get_subscription( $subscription->get_id() )->calculate_totals();

			// Fire custom action hook for hosting updates!
			do_action( 'skyhshoso_subscription_switch_completed', $subscription, $order );
		}
	}
	...
	public static function set_force_payment_flag_in_cart( $total ) {
		...
		if ( $total > 0 || false || false === self::cart_contains_switches( 'any' ) ) {
			return $total;
		}
		...
		foreach ( self::get_recurring_carts() as $cart ) {
		...
		foreach ( WC()->cart->get_cart() as $cart_item_key => $cart_item ) {
			...
			$force_automatic_payments = ! $is_manual_subscription || 'no' === get_option( 'skyhshoso' . '_accept_manual_renewals', 'no' );
	...
	public static function maybe_set_payment_method_after_switch( $order ) {
		...
		foreach ( skyhshoso_get_subscriptions_for_switch_order( $order ) as $subscription ) {
	...
	public static function delay_granting_download_permissions( $order_id ) {
		if ( skyhshoso_order_contains_switch( $order_id ) ) {
			remove_action( 'woocommerce_grant_product_download_permissions', 'wc_downloadable_product_permissions' );
		}
	}
	...
	public static function grant_download_permissions( $order ) {
		...
		add_action( 'woocommerce_grant_product_download_permissions', 'wc_downloadable_product_permissions' );
	}
	...
	public static function calculate_total_paid_since_last_order( $subscription, $subscription_item, $include_sign_up_fees = 'include_sign_up_fees', $orders_to_include = array() ) {
		...
		skyhshoso_sort_objects( $orders, 'date_paid', 'descending' );
		...
		foreach ( $orders as $order ) {
			...
			$order_item = skyhshoso_find_matching_line_item( $order, $subscription_item );
			...
			if ( $has_been_switched && skyhshoso_order_contains_switch( $order ) ) {
				...
				if ( isset( $switched_subscription_items[ $switched_subscription_item_id ] ) ) {
					...
					if ( $switch_order_item_id && (bool) skyhshoso_get_order_item( $switch_order_item_id, $order ) ) {
			...
			if ( $order_is_parent || ( skyhshoso_order_contains_renewal( $order ) && ! skyhshoso_order_contains_early_renewal( $order ) ) ) {
				break;
			}
		}
	...
	public static function display_switches_in_related_order_metabox( $orders_to_display, $subscriptions, $order ) {
		if ( $order instanceof WP_Post ) {
			skyhshoso_deprecated_argument( __METHOD__, '4.7.0', 'Passing a WP Post object is deprecated. This function now expects an Order or Subscription object.' );
		...
		if ( $order instanceof WC_Subscription ) {
			...
			foreach ( skyhshoso_get_switch_orders_for_subscription( $order->get_id() ) as $switch_order ) {
				$switch_order->update_meta_data( '_relationship', __( 'Switch Order', 'skyhs-hosting-solution' ) );
			...
			if ( ! empty( $order->post_parent ) ) {
				$switched_subscriptions = skyhshoso_get_subscriptions_for_switch_order( $order->get_parent_id() );
			}

			// On the Edit Order screen, show any subscriptions with items switched by this order.
		} else {
			$switched_subscriptions = skyhshoso_get_subscriptions_for_switch_order( $order );
		}

		foreach ( $switched_subscriptions as $subscription ) {
			$subscription->update_meta_data( '_relationship', __( 'Switched Subscription', 'skyhs-hosting-solution' ) );
	...
	public static function maybe_set_payment_method( $payment_processing_result, $order_id ) {
		...
		if ( skyhshoso_order_contains_switch( $order_id ) && false != get_post_meta( $order_id, '_paid_date', true ) ) {
	...
	public static function maybe_set_free_trial( $total = '' ) {
		...
		foreach ( WC()->cart->get_cart() as $cart_item_key => $cart_item ) {
			if ( isset( $cart_item['subscription_switch']['first_payment_timestamp'] ) && 0 != $cart_item['subscription_switch']['first_payment_timestamp'] ) {
				WC()->cart->cart_contents[ $cart_item_key ]['data']->update_meta_data( 'subscription_trial_length', 1 );
			}
		}
	...
	public static function maybe_unset_free_trial( $total = '' ) {
		...
		foreach ( WC()->cart->get_cart() as $cart_item_key => $cart_item ) {
			if ( isset( $cart_item['subscription_switch']['first_payment_timestamp'] ) && 0 != $cart_item['subscription_switch']['first_payment_timestamp'] ) {
				WC()->cart->cart_contents[ $cart_item_key ]['data']->update_meta_data( 'subscription_trial_length', 0 );
			}
		}
		return $total;
	}
	...
	protected static function has_different_billing_schedule( $cart_item, $subscription ) {
		return SkyHSHOSO_Subscriptions_Product::get_period( $cart_item['data'] ) != $subscription->get_billing_period() || SkyHSHOSO_Subscriptions_Product::get_interval( $cart_item['data'] ) != $subscription->get_billing_interval();
	}
	...
	protected static function has_different_length( $recurring_cart, $subscription ) {
		$recurring_cart_end_date = gmdate( 'Y-m-d', skyhshoso_date_to_time( $recurring_cart->end_date ) );
	...
	public static function cart_contains_subscription_creating_switch() {
		...
		foreach ( self::get_recurring_carts() as $recurring_cart_key => $recurring_cart ) {
	...
	public static function display_switch_add_to_cart_text( $add_to_cart_text ) {
		if ( isset( $_GET['switch-subscription'], $_GET['item'] ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Recommended
			$add_to_cart_text = _x( 'Switch subscription', 'add to cart button text while switching a subscription', 'skyhs-hosting-solution' );
		}
	...
	public static function add_switch_button( $all_actions, $subscriptions ) {
		...
		foreach ( $all_actions as $subscription_key => $actions ) {
			...
			if ( false ) { // Deprecated — replaced by can_item_be_switched()
				$all_actions[ $subscription_key ] = array(
					'switch' => array(
						'url'  => self::get_switch_link( $subscription_key ),
						'name' => get_option( 'skyhshoso' . '_switch_button_text', __( 'Upgrade or Downgrade', 'skyhs-hosting-solution' ) ),
					),
	...
	public static function calculate_first_payment_date( $next_payment_date, $order, $product_id, $type ) {
		_deprecated_function( __METHOD__, '2.0' );
		return self::get_first_payment_date( $next_payment_date, $order->get_id() . '_' . $product_id, $order->get_user_id(), $type );
	}
	...
	public static function get_first_payment_date( $next_payment_date, $subscription_key, $user_id, $type ) {
		...
		$subscription = skyhshoso_get_subscription_from_key( $subscription_key );

		if ( $subscription->has_status( 'active' ) && $subscription->get_parent_id() && skyhshoso_order_contains_switch( $subscription->get_parent_id() ) && 1 >= $subscription->get_payment_count() ) {
	...
	public static function add_switched_status_string( $status_string ) {
		...
		if ( 'switched' === strtolower( $status_string ) ) {
			$status_string = _x( 'Switched', 'Subscription status', 'skyhs-hosting-solution' );
		}
	...
	public static function order_contains_subscription_switch( $order_id ) {
		_deprecated_function( __METHOD__, '2.0', 'skyhshoso_order_contains_switch( $order_id )' );
		return skyhshoso_order_contains_switch( $order_id );
	}
	...
	public static function set_switch_order_item_id( $order_id, $posted_checkout_data ) {
		_deprecated_function( __METHOD__, '2.2.1', 'SkyHSHOSO_Cart_Switch::set_cart_item_order_item_id()' );
	...
	public static function add_related_orders( $related_orders, $subscription, $return_fields, $order_type ) {
		skyhshoso_deprecated_function( __METHOD__, '2.3.0', 'skyhshoso_get_switch_orders_for_subscription()' );
		if ( in_array( $order_type, array( 'all', 'switch' ) ) ) {

			$switch_orders = skyhshoso_get_switch_orders_for_subscription( $subscription->get_id() );
	...
	public static function switch_order_meta_box_rows( $post ) {
		skyhshoso_deprecated_function( __METHOD__, '3.1.0' );
		$subscriptions          = array();
		$switched_subscriptions = array();
		$orders                 = array();

		// On the subscription page, just show related orders
		if ( skyhshoso_get_subscription( $post->ID ) ) {

			// Select the orders which switched item/s from this subscription
			$orders = skyhshoso_get_switch_orders_for_subscription( $post->ID );

			foreach ( $orders as $order_id => $order ) {
				$order->update_meta_data( 'relationship', __( 'Switch Order', 'skyhs-hosting-solution' ) );
			}

			// Select the subscriptions which had item/s switched to this subscription by its parent order
			if ( ! empty( $post->post_parent ) ) {
				$switched_subscriptions = skyhshoso_get_subscriptions_for_switch_order( $post->post_parent );
			}

		// On the Edit Order screen, show any subscriptions with items switched by this order
		} else {
			$switched_subscriptions = skyhshoso_get_subscriptions_for_switch_order( $post->ID );
		}

		if ( is_array( $switched_subscriptions ) ) {
			foreach ( $switched_subscriptions as $subscription_id => $subscription ) {
				$subscription->update_meta_data( 'relationship', __( 'Switched Subscription', 'skyhs-hosting-solution' ) );
				$orders[ $subscription_id ] = $subscription;
			}
		}

		foreach ( $orders as $order ) {
			include( SKYHSHOSO_ABSPATH . 'includes/admin/meta-boxes/views/html-related-orders-row.php' );
		}
