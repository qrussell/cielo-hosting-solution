...
class SkyHSHOSO_Early_Renewal_Manager {
	...
	public static function init() {
		self::$setting_id           = SkyHSHOSO_Admin::$option_prefix . '_enable_early_renewal';
		self::$via_modal_setting_id = SkyHSHOSO_Admin::$option_prefix . '_enable_early_renewal_via_modal';
	...
	public static function add_settings( $settings ) {
		$early_renewal_settings = array(
			array(
				'id'              => self::$setting_id,
				'name'            => __( 'Early Renewal', 'skyhs-hosting-solution' ),
				'desc'            => __( 'Accept Early Renewal Payments', 'skyhs-hosting-solution' ),
				'desc_tip'        => __( 'With early renewals enabled, customers can renew their subscriptions before the next payment date.', 'skyhs-hosting-solution' ),
				'default'         => 'no',
				'type'            => 'checkbox',
				'checkboxgroup'   => 'start',
			...
			array(
				...
				'desc'            => __( 'Accept Early Renewal Payments via a Modal', 'skyhs-hosting-solution' ),
				'desc_tip'        => sprintf(
					/* translators: 1-2: opening/closing <strong> tags , 2-3: opening/closing tags for a link to docs on early renewal. */
					__( 'Allow customers to bypass the checkout and renew their subscription early from their %1$sMy Account > View Subscription%2$s page. %3$sLearn more.%4$s', 'skyhs-hosting-solution' ),
					'<strong>', '</strong>',
		...
		SkyHSHOSO_Admin::insert_setting_after( $settings, 'skyhs_hosting_solution_turn_off_automatic_payments', $early_renewal_settings, 'multiple_settings' );
	...
	public static function is_early_renewal_enabled() {
		...
		if ( false === $enabled ) {
			$enabled = skyhshoso_do_subscriptions_exist() ? 'no' : 'yes';
			update_option( self::$setting_id, $enabled );
		}

		return apply_filters( 'skyhshoso_is_early_renewal_enabled', 'yes' === $enabled );
	}
	...
	public static function is_early_renewal_via_modal_enabled() {
		return self::is_early_renewal_enabled() && apply_filters( 'skyhshoso_is_early_renewal_via_modal_enabled', 'yes' === get_option( self::$via_modal_setting_id, 'no' ) );
	}
	 ...
	 * @since 2.6.0
	 *
	 * @param SkyHSHOSO_Subscription $subscription The subscription to calculate the dates for.
	...
	public static function get_dates_to_update( $subscription ) {
		...
		if ( $next_payment_time > 0 && $next_payment_time > current_time( 'timestamp', true ) ) {
			$next_payment_timestamp = skyhshoso_add_time( $subscription->get_billing_interval(), $subscription->get_billing_period(), $next_payment_time );
			...
			$dates_to_update['end'] = gmdate( 'Y-m-d H:i:s', skyhshoso_add_time( $subscription->get_billing_interval(), $subscription->get_billing_period(), $subscription->get_time( 'end' ) ) );
		}
