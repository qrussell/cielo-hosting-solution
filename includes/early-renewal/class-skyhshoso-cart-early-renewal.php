 ...
 *
 * @class      SkyHSHOSO_Cart_Early_Renewal
 * @package    SkyHS Hosting Solution
 * @subpackage SkyHSHOSO_Early_Renewal
 * @category   Class
 * @since      2.3.0
 */
if ( ! defined( 'ABSPATH' ) ) {
...
class SkyHSHOSO_Cart_Early_Renewal extends SkyHSHOSO_Cart_Renewal {
	 ...
	 * @var string
	 */
	const SUBSCRIPTION_DATES_UPDATED_META_KEY = '_skyhshoso_early_renewal_subscription_dates_updated';
	...
	public function __construct() {
		...
		add_filter( 'skyhshoso_view_subscription_actions', array( $this, 'add_renew_now_action' ), 10, 2 );
		...
		// After the renewal order is created on checkout, set the renewal order cart item data now that we have an order. Must be hooked on before SkyHSHOSO_Cart_Renewal->set_order_item_id(), in order for the line item ID set by that function to be correct.
		...
		add_action( 'skyhshoso_before_early_renewal_setup_cart_subscription', 'skyhshoso_allow_protected_products_to_renew' );
		add_action( 'skyhshoso_after_early_renewal_setup_cart_subscription', 'skyhshoso_disallow_protected_product_add_to_cart_validation' );
	}
	 ...
	 * @param SkyHSHOSO_Subscription $subscription The current subscription being viewed.
	 * @since 2.3.0
	...
	public function add_renew_now_action( $actions, $subscription ) {
		...
		if ( skyhshoso_can_user_renew_early( $subscription ) && $subscription->payment_method_supports( 'subscription_date_changes' ) && $subscription->has_status( 'active' ) ) {

			$actions['subscription_renewal_early'] = array(
				'url'  => skyhshoso_get_early_renewal_url( $subscription ),
				'name' => __( 'Renew now', 'skyhs-hosting-solution' ),
			);
		}
	...
	public function maybe_setup_cart() {
		...
		if ( ! isset( $_GET['_wpnonce'] ) || ! wp_verify_nonce( sanitize_key( $_GET['_wpnonce'] ), 'skyhshoso_early_renewal_' . $subscription_id ) ) {
			wc_add_notice( __( 'Security check failed. Please try again.', 'skyhs-hosting-solution' ), 'error' );
		...
		if ( empty( $subscription ) ) {
			...
			wc_add_notice( __( 'That subscription does not exist. Has it been deleted?', 'skyhs-hosting-solution' ), 'error' );

		} elseif ( (int) $subscription->get_customer_id() !== (int) get_current_user_id() && ! current_user_can( 'manage_woocommerce' ) ) {

			wc_add_notice( __( "That doesn't appear to be one of your subscriptions.", 'skyhs-hosting-solution' ), 'error' );

		} elseif ( ! skyhshoso_can_user_renew_early( $subscription ) ) {

			wc_add_notice( __( 'You can not renew this subscription early. Please contact us if you need assistance.', 'skyhs-hosting-solution' ), 'error' );

		} else {

			do_action( 'skyhshoso_before_early_renewal_setup_cart_subscription', $subscription );

			wc_add_notice( __( 'Complete checkout to renew now.', 'skyhs-hosting-solution' ), 'success' );
			...
			do_action( 'skyhshoso_after_early_renewal_setup_cart_subscription', $subscription );
	...
	public function copy_subscription_meta_to_order( $order ) {
		if ( $this->cart_contains() ) {
			...
			if ( $subscription ) {
				...
				skyhshoso_copy_order_meta( $subscription, $order, 'renewal_order' );
	...
	public function add_early_renewal_metadata_to_order( $order, $data = array() ) {
		...
		// Get the subscription.
		$subscription = skyhshoso_get_subscription( $cart_item[ $this->cart_item_key ]['subscription_id'] );

		// Mark this order as a renewal.
		SkyHSHOSO_Related_Order_Store::instance()->add_relation( $order, $subscription, 'renewal' );
	 ...
	 * @return SkyHSHOSO_Subscription The subscription object.
	 * @since  2.3.0
	 */
	...
	public function add_note_to_record_early_renewal( $order_id ) {
		...
		if ( skyhshoso_is_order( $order ) && skyhshoso_is_subscription( $subscription ) ) {
			// translators: %s: order ID.
			$order_number = sprintf( _x( '#%s', 'hash before order number', 'skyhs-hosting-solution' ), $order->get_order_number() );

			// translators: %s: order ID (linked to details page).
			$subscription->add_order_note( sprintf( __( 'Order %s created to record early renewal.', 'skyhs-hosting-solution' ), sprintf( '<a href="%s">%s</a> ', esc_url( skyhshoso_get_edit_post_link( $order_id ) ), $order_number ) ) );
		}
	}
	 ...
	 * created on checkout. Required so the line item ID set by @see SkyHSHOSO_Cart_Renewal->set_order_item_id()
	 * matches the order.
	 *
	...
	public function set_cart_item_renewal_order_data( $order_id ) {
		...
		if ( ! skyhshoso_cart_contains_early_renewal() ) {
			return;
		}
	 ...
	 *
	 * This function is intended to run after @see SkyHSHOSO_Cart_Renewal::filter_my_account_my_orders_actions() which removes the cancel and pay option.
	 *
	...
	public static function filter_early_renewal_order_actions( $actions, $order ) {
		...
		if ( ! skyhshoso_order_contains_early_renewal( $order ) ) {
			return $actions;
		}
		...
		if ( ! isset( $actions['cancel'] ) && in_array( $order->get_status(), apply_filters( 'woocommerce_valid_order_statuses_for_cancel', array( 'pending', 'failed' ), $order ) ) ) {
			...
			if ( skyhshoso_is_view_subscription_page() ) {
				global $wp;
				$subscription = skyhshoso_get_subscription( $wp->query_vars['view-subscription'] );
			...
			$actions['cancel'] = array(
				...
				'name' => __( 'Cancel', 'skyhs-hosting-solution' ),
			);
		}
	 ...
	 *
	 * Renewal orders are usually not cancellable because @see SkyHSHOSO_Renewal_Order::prevent_cancelling_renewal_orders() prevents the request from being processed.
	...
	public static function allow_early_renewal_order_cancellation() {
		if ( isset( $_GET['cancel_order'], $_GET['order_id'], $_GET['_wpnonce'] ) && wp_verify_nonce( sanitize_text_field( wp_unslash( $_GET['_wpnonce'] ) ), 'woocommerce-cancel_order' ) ) {
			...
			if ( skyhshoso_order_contains_early_renewal( $order ) ) {
				remove_action( 'wp_loaded', 'SkyHSHOSO_Renewal_Order::prevent_cancelling_renewal_orders', 19 );
			}
		}
	}
	 ...
	 *
	 * Attached to the dynamic hook 'skyhshoso_renewal_order_meta' which is triggered by skyhshoso_copy_order_meta
	...
	public function exclude_core_order_meta_properties( $order_meta ) {
		skyhshoso_deprecated_function( __METHOD__, '4.8.0', __CLASS__ . '::exclude_core_properties_from_copy()' );

		// Additional meta keys to exclude. These are in addition to the meta keys already excluded by skyhshoso_copy_order_meta().
	...
	public function maybe_record_subscription_payment( $order_id, $old_status, $new_status, $order = null ) {
		...
		if ( remove_action( 'woocommerce_order_status_changed', 'SkyHSHOSO_Renewal_Order::maybe_record_subscription_payment', 10 ) ) {
		...
		$subscription = skyhshoso_get_subscription( absint( $order->get_meta( '_subscription_renewal_early' ) ) );
		...
		if ( $subscription && $order->is_paid() && ! $order->meta_exists( self::SUBSCRIPTION_DATES_UPDATED_META_KEY ) ) {
			...
			skyhshoso_update_dates_after_early_renewal( $subscription, $order );
	...
	public function reattach_renewal_order_status_handling() {
		add_action( 'woocommerce_order_status_changed', 'SkyHSHOSO_Renewal_Order::maybe_record_subscription_payment', 10, 3 );
	}
	...
	public function maybe_update_dates( $order_id ) {
		skyhshoso_deprecated_function( __METHOD__, '5.2.0' );
		...
		$subscription_id = skyhshoso_get_objects_property( $order, 'subscription_renewal_early' );
		$subscription    = skyhshoso_get_subscription( $subscription_id );
	...
	public function maybe_reactivate_subscription( $order_id ) {
		...
		if ( skyhshoso_order_contains_early_renewal( $order ) ) {
			...
			$subscription = skyhshoso_get_subscription( skyhshoso_get_objects_property( $order, 'subscription_renewal_early' ) );
	...
	public function maybe_record_early_renewal( $order_id, $posted_data ) {
		skyhshoso_deprecated_function( __METHOD__, '5.2.0', 'SkyHSHOSO_Cart_Early_Renewal::add_early_renewal_metadata_to_order( $order, $posted_data )' );
		...
		$subscription->update_status( 'on-hold', _x( 'Customer requested to renew early:', 'used in order note as reason for why subscription status changed', 'skyhs-hosting-solution' ) );
	}

	/**
	 * Ensure customers can cancel early renewal orders.
	 *
	 * Renewal orders are usually not cancellable because @see SkyHSHOSO_Cart_Renewal::filter_my_account_my_orders_actions() prevents it.
	 * In the case of early renewals, the customer has opted for early renewal and so should be able to cancel it.
	 *
	 * @param array $actions A list of actions customers can make on an order from their My Account page
	...
	public static function add_cancel_order_action( $actions, $order ) {
		skyhshoso_deprecated_function( __METHOD__, '5.6.0', __CLASS__ . '::filter_early_renewal_order_actions()' );

		if ( ! isset( $actions['cancel'] ) && skyhshoso_order_contains_early_renewal( $order ) && in_array( $order->get_status(), apply_filters( 'woocommerce_valid_order_statuses_for_cancel', array( 'pending', 'failed' ), $order ) ) ) {
			$redirect = wc_get_page_permalink( 'myaccount' );
