 ...
 * @subpackage     SkyHSHOSO_Retry_Manager
 * @category       Class
 * @since          2.1
 */

class SkyHSHOSO_Retry_Manager {
	...
	/* property to store the instance of SkyHSHOSO_Retry_Admin */
	protected static $admin;

	/**
	 * Background updater to process retries from old store.
	 *
	 * @var SkyHSHOSO_Retry_Background_Migrator
	 */
	 ...
	 *
	 * @var SkyHSHOSO_Retry_Table_Maker
	 */
	...
	public static function init() {
		self::$setting_id = 'skyhshoso_enable_retry';
		self::$admin      = new SkyHSHOSO_Retry_Admin( self::$setting_id );

		if ( self::is_retry_enabled() ) {
			SkyHSHOSO_Retry_Email::init();
			...
			add_filter( 'skyhshoso_subscription_dates', __CLASS__ . '::add_retry_date_type' );

			add_action( 'skyhshoso_subscription_status_updated', __CLASS__ . '::maybe_cancel_retry', 0, 3 );

			add_action( 'skyhshoso_retry_status_updated', __CLASS__ . '::maybe_delete_payment_retry_date', 0, 2 );
			...
			add_filter( 'skyhshoso_is_failed_renewal_order', __CLASS__ . '::compare_order_and_retry_statuses', 10, 3 );
			...
			if ( ! self::$table_maker ) {
				self::$table_maker = new SkyHSHOSO_Retry_Table_Maker();
	...
	public static function attach_wc_dependant_hooks() {
		$hpos_enabled = class_exists( '\Automattic\WooCommerce\Utilities\FeaturesUtil' ) && \Automattic\WooCommerce\Utilities\FeaturesUtil::feature_is_enabled( 'custom_order_tables' );
	...
	public static function check_order_statuses_for_payment( $statuses, $order = null ) {
		...
		$last_retry = self::store()->get_last_retry_for_order( $order->get_id() );
		if ( $last_retry ) {
	...
	public static function is_retry_enabled() {
		return (bool) apply_filters( 'skyhshoso_is_retry_enabled', 'yes' == get_option( self::$setting_id, 'no' ) );
	}
	...
	public static function add_retry_date_type( $subscription_date_types ) {
		...
		$new_types = array();
		foreach ( $subscription_date_types as $key => $label ) {
			$new_types[ $key ] = $label;
			if ( 'next_payment' === $key ) {
				$new_types['payment_retry'] = _x( 'Renewal Payment Retry', 'table heading', 'skyhs-hosting-solution' );
			}
		}
		$subscription_date_types = $new_types;
	 ...
	 *
	 * @param object $subscription An instance of a SkyHSHOSO_Subscription object
	...
	public static function maybe_cancel_retry( $subscription, $new_status, $old_status ) {
		...
		$payment_retry = $subscription->get_meta( 'payment_retry' );

		if ( ! empty( $payment_retry ) && strtotime( $payment_retry ) > 0 ) {

			// Get the most recent renewal order for this subscription.
			...
			$last_order = ! empty( $renewal_orders ) ? current( $renewal_orders ) : wc_get_order( $subscription->get_order_id() );
			$last_retry = ( $last_order ) ? self::store()->get_last_retry_for_order( $last_order->get_id() ) : null;
			...
			if ( null !== $last_retry && 'cancelled' !== $last_retry->get_status() && null !== ( $last_retry_rule = $last_retry->get_rule() ) ) {
				...
				$applying_retry_rule       = did_action( 'skyhshoso_before_apply_retry_rule' ) !== did_action( 'skyhshoso_after_apply_retry_rule' );
				...
				if ( $new_status != $retry_subscription_status && ! $applying_retry_rule && ! $retrying_payment ) {
					...
					SkyHSHOSO_Subscription_DB::delete_meta( $subscription->get_id(), 'payment_retry' );
				}
			}
		}
	}
	 ...
	 *
	 * @param object $retry An instance of a SkyHSHOSO_Retry object
	...
	public static function maybe_delete_payment_retry_date( $retry, $new_status ) {
		if ( ! in_array( $new_status, array( 'pending', 'processing' ) ) ) {
			...
			if ( $retry->get_id() === $last_retry->get_id() ) {
				$subscription_rows = SkyHSHOSO_Subscription_DB::get_by_order( $retry->get_order_id() );
	 ...
	 *
	 * @param SkyHSHOSO_Subscription $subscription The subscription on which the payment failed.
	...
	public static function maybe_apply_retry_rule( $subscription, $last_order ) {
		// SkyHS only supports automatic subscriptions with payment methods that support date changes.
		if ( ! self::is_scheduled_payment_attempt() ) {
			return;
		}

		$retry_count = self::store()->get_retry_count_for_order( $last_order->get_id() );

		if ( self::rules()->has_rule( $retry_count, $last_order->get_id() ) ) {

			$retry_rule = self::rules()->get_rule( $retry_count, $last_order->get_id() );

			do_action( 'skyhshoso_before_apply_retry_rule', $retry_rule, $last_order, $subscription );

			$retry_id = self::store()->save( new SkyHSHOSO_Retry( array(
				'status'   => 'pending',
			...
			foreach ( array( 'order' => $last_order, 'subscription' => $subscription ) as $object_key => $object ) { // phpcs:ignore WordPress.Arrays.ArrayDeclarationSpacing.AssociativeArrayFound
				...
				if ( '' !== $new_status && ! $object->has_status( $new_status ) ) {
					$object->update_status( $new_status, _x( 'Retry rule applied:', 'used in order note as reason for why status changed', 'skyhs-hosting-solution' ) );
				}
			}

			if ( $retry_rule->get_retry_interval() > 0 ) {
				// Store the payment retry date on the subscription as meta.
				$payment_retry_date = gmdate( 'Y-m-d H:i:s', gmdate( 'U' ) + $retry_rule->get_retry_interval( $retry_count ) );
				$subscription->update_meta_data( 'payment_retry', $payment_retry_date );
				$subscription->save();

				// Schedule the retry action.
				if ( function_exists( 'as_schedule_single_action' ) ) {
					as_schedule_single_action(
						gmdate( 'U' ) + $retry_rule->get_retry_interval( $retry_count ),
						'woocommerce_scheduled_subscription_payment_retry',
						array( $last_order->get_id() )
					);
				}
			}

			do_action( 'skyhshoso_after_apply_retry_rule', $retry_rule, $last_order, $subscription );
		}
	}
	...
	public static function maybe_reapply_last_retry_rule( $subscription, $last_order ) {
		...
		foreach ( array( 'order' => $last_order, 'subscription' => $subscription ) as $object_type => $object ) { // phpcs:ignore WordPress.Arrays.ArrayDeclarationSpacing.AssociativeArrayFound
			...
			if ( '' !== $new_status && ! $object->has_status( $new_status ) ) {
				$object->update_status( $new_status, _x( 'Retry rule reapplied:', 'used in order note as reason for why status changed', 'skyhs-hosting-solution' ) );
			}
		}
	}
	...
	public static function maybe_retry_payment( $order_id ) {
		...
		$subscription_rows = SkyHSHOSO_Subscription_DB::get_by_order( $last_order->get_id() );
		$subscriptions     = array();
		foreach ( $subscription_rows as $row ) {
			$subscriptions[ (int) $row->id ] = new SkyHSHOSO_Subscription( $row );
		}
		...
		if ( null !== $last_retry && 'pending' === $last_retry->get_status() ) {
			...
			if ( $last_order->needs_payment() ) {
				...
				if ( $valid_order_status && $valid_subscription_status ) {
					...
					$last_order->update_status( 'pending', _x( 'Subscription renewal payment retry:', 'used in order note as reason for why order status changed', 'skyhs-hosting-solution' ), true );
					...
					foreach ( $subscriptions as $subscription ) {
						...
						$subscription->update_status( 'on-hold', _x( 'Subscription renewal payment retry:', 'used in order note as reason for why subscription status changed', 'skyhs-hosting-solution' ) );

						// Store a hash of the payment method and payment meta to determine if there's a single payment method being used.
						$payment_meta_hash = md5( $subscription->get_payment_method() . '[]' );
						$unique_payment_methods[ $payment_meta_hash ] = 1;
					}

					// SkyHS does not support manual subscriptions — always use the subscription's payment method.
					if ( 1 < count( $unique_payment_methods ) ) {
						...
						throw new Exception( esc_html( __( 'Payment retry attempted on renewal order with multiple related subscriptions with no payment method in common.', 'skyhs-hosting-solution' ) ) );
					} else {
						...
						$last_order->set_payment_method( $subscription->get_payment_method() );
						$last_order->save();

						// Trigger the payment gateway hook — same hook the SkyHS cron uses for normal renewals.
						$method = $last_order->get_payment_method();
						$total  = $last_order->get_total();
						do_action( 'woocommerce_scheduled_subscription_payment_' . $method, $total, $last_order );

						// Now that we've attempted to process the payment, refresh the order
						$last_order = wc_get_order( $last_order->get_id() );
					}
	...
	*/
	...
	protected static function is_scheduled_payment_attempt() {
		 ...
		 * @since 2.5.0
		 */
		return (bool) apply_filters( 'skyhshoso_is_scheduled_payment_attempt', doing_action( 'woocommerce_scheduled_subscription_payment' ) || doing_action( 'woocommerce_scheduled_subscription_payment_retry' ) );
	}
	...
	public static function store() {
		if ( empty( self::$store ) ) {
			if ( ! did_action( 'plugins_loaded' ) ) {
				wc_doing_it_wrong( __METHOD__, 'This method was called before the "plugins_loaded" hook. It applies a filter to the retry data store instantiated. For that to work, it should first be called after all plugins are loaded.', '2.4.1' );
			}
	...
	protected static function get_store_class() {
		$default_store_class = 'SkyHSHOSO_Retry_Database_Store';
		if ( SkyHSHOSO_Retry_Migrator::needs_migration() ) {
			$default_store_class = 'SkyHSHOSO_Retry_Hybrid_Store';
		}

		return apply_filters( 'skyhshoso_retry_store_class', $default_store_class );
	}
	...
	protected static function get_rules_class() {
		return apply_filters( 'skyhshoso_retry_rules_class', 'SkyHSHOSO_Retry_Rules' );
	}
